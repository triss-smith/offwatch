import type { HeartbeatRun, HeartbeatRunEvent, InstanceSchedulerHeartbeatAgent, WorkspaceOperation } from "@offwatch/shared";
...
export const heartbeatsApi = {
  list: (workspaceId: string, agentId?: string, limit?: number) => {
    const searchParams = new URLSearchParams();
    if (agentId) searchParams.set("agentId", agentId);
    if (limit) searchParams.set("limit", String(limit));
    const qs = searchParams.toString();
    return api.get<HeartbeatRun[]>(`/workspaces/${workspaceId}/heartbeat-runs${qs ? `?${qs}` : ""}`);
  },
  ...
  liveRunsForCompany: (workspaceId: string, minCount?: number) =>
    api.get<LiveRunForIssue[]>(`/workspaces/${workspaceId}/live-runs${minCount ? `?minCount=${minCount}` : ""}`),
