import type { AssetImage } from "@offwatch/shared";
import { api } from "./client";

export const assetsApi = {
  uploadImage: async (workspaceId: string, file: File, namespace?: string) => {
    ...
    return api.postForm<AssetImage>(`/workspaces/${workspaceId}/assets/images`, form);
  },

  uploadCompanyLogo: async (workspaceId: string, file: File) => {
    const buffer = await file.arrayBuffer();
    const safeFile = new File([buffer], file.name, { type: file.type });

    const form = new FormData();
    form.append("file", safeFile);
    return api.postForm<AssetImage>(`/workspaces/${workspaceId}/logo`, form);
  },
};
