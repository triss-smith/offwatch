    ...
    <div className="mx-auto max-w-xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          {hasActiveInvite
            ? "No instance admin exists yet. A bootstrap invite is already active. Check your Offwatch startup logs for the first admin invite URL, or run this command to rotate it:"
            : "No instance admin exists yet. Run this command in your Offwatch environment to generate the first admin invite URL:"}
        </p>
        <pre className="mt-4 overflow-x-auto rounded-md border border-border bg-muted/30 p-3 text-xs">
{`pnpm offwatchai auth bootstrap-ceo`}
        </pre>
      </div>
    </div>
  );
      ...
      <div className="rounded-lg border border-border bg-card p-6">
        ...
        <div className="mt-4">
          <Button
            onClick={() =>
              matchedCompany
                ? openOnboarding({ initialStep: 2, workspaceId: matchedCompany.id })
                : openOnboarding()
            }
          >
...
function CompanyRootRedirect() {
  const { companies, selectedCompany, loading } = useWorkspace();
  const location = useLocation();
