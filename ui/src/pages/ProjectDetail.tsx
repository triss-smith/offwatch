...
import { PROJECT_COLORS, isUuidLike, type BudgetPolicySummary, type ExecutionWorkspace } from "@offwatch/shared";
...
function ProjectIssuesList({ projectId, workspaceId }: { projectId: string; workspaceId: string }) {
  ...
  return (
    <IssuesList
      ...
      projectId={projectId}
      viewStateKey={`offwatch:project-view:${projectId}`}
...
export function ProjectDetail() {
  ...
  if (routeProjectRef && activeTab === null) {
    ...
    if (project?.id) {
      try { cachedTab = localStorage.getItem(`offwatch:project-tab:${project.id}`); } catch {}
    }
  ...
  const handleTabChange = (tab: ProjectTab) => {
    ...
    if (project?.id) {
      try { localStorage.setItem(`offwatch:project-tab:${project.id}`, tab); } catch {}
    }
