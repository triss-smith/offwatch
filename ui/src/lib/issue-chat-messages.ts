...
import type { Agent, IssueComment } from "@offwatch/shared";
...
function createCommentMessage(args: {
  ...
  const { comment, agentMap, currentUserId, workspaceId, projectId } = args;
...
export function buildIssueChatMessages(args: {
  ...
  for (const comment of sortByCreated(comments)) {
    orderedMessages.push({
      ...
      order: 1,
      message: createCommentMessage({ comment, agentMap, currentUserId, workspaceId, projectId }),
    });
  }
