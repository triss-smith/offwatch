...
function bytesToPortableFileEntry(pathValue: string, bytes: Uint8Array): WorkspacePortabilityFileEntry {
...
function portableFileEntryToBytes(entry: WorkspacePortabilityFileEntry): Uint8Array {
...
export async function readZipArchive(source: ArrayBuffer | Uint8Array): Promise<{
  ...
  const entries: Array<{ path: string; body: WorkspacePortabilityFileEntry }> = [];
  let offset = 0;
...
export function createZipArchive(files: Record<string, WorkspacePortabilityFileEntry>, rootPath: string): Uint8Array {
