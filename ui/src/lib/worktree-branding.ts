...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("offwatch-worktree-enabled") !== "true") return null;

  const name = readMetaContent("offwatch-worktree-name");
  const color = normalizeHexColor(readMetaContent("offwatch-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("offwatch-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
