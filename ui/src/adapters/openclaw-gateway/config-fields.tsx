...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="Offwatch API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "offwatchApiUrl",
                  String(config.offwatchApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "offwatchApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://offwatch.example"
            />
          </Field>
          ...
          <Field label="Claimed API key path">
            <DraftInput
              ...
              immediate
              className={inputClass}
              placeholder="~/.openclaw/workspace/offwatch-claimed-api-key.json"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "offwatch"))}
                onCommit={(v) => mark("adapterConfig", "sessionKey", v || undefined)}
                immediate
                className={inputClass}
                placeholder="offwatch"
              />
            </Field>
          )}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. Offwatch persists a device key during onboarding so pairing approvals
