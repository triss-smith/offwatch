...
import type { CreateConfigValues } from "@offwatch/adapter-utils";

// Re-export shared types so local consumers don't need to change imports
export type { TranscriptEntry, StdoutLineParser, CreateConfigValues } from "@offwatch/adapter-utils";

export interface StatefulStdoutParser {
  parseLine: (line: string, ts: string) => import("@offwatch/adapter-utils").TranscriptEntry[];
  reset: () => void;
}
...
export interface TranscriptParserSource {
  parseStdoutLine: (line: string, ts: string) => import("@offwatch/adapter-utils").TranscriptEntry[];
