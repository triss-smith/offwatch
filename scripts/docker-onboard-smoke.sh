...
IMAGE_NAME="${IMAGE_NAME:-offwatch-onboard-smoke}"
HOST_PORT="${HOST_PORT:-3131}"
OFFWATCHAI_VERSION="${OFFWATCHAI_VERSION:-latest}"
...
OFFWATCH_DEPLOYMENT_MODE="${OFFWATCH_DEPLOYMENT_MODE:-authenticated}"
OFFWATCH_DEPLOYMENT_EXPOSURE="${OFFWATCH_DEPLOYMENT_EXPOSURE:-private}"
OFFWATCH_PUBLIC_URL="${OFFWATCH_PUBLIC_URL:-http://localhost:${HOST_PORT}}"
SMOKE_AUTO_BOOTSTRAP="${SMOKE_AUTO_BOOTSTRAP:-true}"
SMOKE_ADMIN_NAME="${SMOKE_ADMIN_NAME:-Smoke Admin}"
SMOKE_ADMIN_EMAIL="${SMOKE_ADMIN_EMAIL:-smoke-admin@offwatch.local}"
SMOKE_ADMIN_PASSWORD="${SMOKE_ADMIN_PASSWORD:-offwatch-smoke-password}"
...
write_metadata_file() {
  ...
  mkdir -p "$(dirname "$SMOKE_METADATA_FILE")"
  {
    ...
    printf 'SMOKE_OFFWATCHAI_VERSION=%q\n' "$OFFWATCHAI_VERSION"
  } >"$SMOKE_METADATA_FILE"
}
...
generate_bootstrap_invite_url() {
  ...
  if bootstrap_output="$(
    docker exec \
      -e OFFWATCH_DEPLOYMENT_MODE="$OFFWATCH_DEPLOYMENT_MODE" \
      -e OFFWATCH_DEPLOYMENT_EXPOSURE="$OFFWATCH_DEPLOYMENT_EXPOSURE" \
      -e OFFWATCH_PUBLIC_URL="$OFFWATCH_PUBLIC_URL" \
      -e OFFWATCH_HOME="/offwatch" \
      "$CONTAINER_NAME" bash -lc \
      'timeout 20s npx --yes "offwatchai@${OFFWATCHAI_VERSION}" auth bootstrap-ceo --data-dir "$OFFWATCH_HOME" --base-url "$OFFWATCH_PUBLIC_URL"' \
      2>&1
  )"; then
    bootstrap_status=0
  else
...
sign_up_or_sign_in() {
  ...
  signup_status="$(post_json_with_cookies \
    "$OFFWATCH_PUBLIC_URL/api/auth/sign-up/email" \
  ...
  signin_status="$(post_json_with_cookies \
    "$OFFWATCH_PUBLIC_URL/api/auth/sign-in/email" \
...
auto_bootstrap_authenticated_smoke() {
  local health_url="$OFFWATCH_PUBLIC_URL/api/health"
  local health_json
  ...
  if [[ "$health_json" == *'"bootstrapStatus":"ready"'* ]]; then
    ...
    accept_status="$(post_json_with_cookies \
      "$OFFWATCH_PUBLIC_URL/api/invites/$invite_token/accept" \
      '{"requestType":"human"}' \
      "$accept_response")"
  ...
  local session_json
  session_json="$(get_with_cookies "$OFFWATCH_PUBLIC_URL/api/auth/get-session")"
  ...
  companies_json="$(get_with_cookies "$OFFWATCH_PUBLIC_URL/api/companies")"
...
docker build \
  --build-arg OFFWATCHAI_VERSION="$OFFWATCHAI_VERSION" \
...
echo "    Public URL: $OFFWATCH_PUBLIC_URL"
echo "    Smoke auto-bootstrap: $SMOKE_AUTO_BOOTSTRAP"
echo "    Detached mode: $SMOKE_DETACH"
echo "    Data dir: $DATA_DIR"
echo "    Deployment: $OFFWATCH_DEPLOYMENT_MODE/$OFFWATCH_DEPLOYMENT_EXPOSURE"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/offwatch-onboard-smoke.XXXXXX")"
COOKIE_JAR="$TMP_DIR/cookies.txt"

if ! wait_for_http "$OFFWATCH_PUBLIC_URL/api/health" 90 1; then
  echo "Smoke bootstrap failed: server did not become ready at $OFFWATCH_PUBLIC_URL/api/health" >&2
  exit 1
fi

if [[ "$SMOKE_AUTO_BOOTSTRAP" == "true" && "$OFFWATCH_DEPLOYMENT_MODE" == "authenticated" ]]; then
  auto_bootstrap_authenticated_smoke
fi
...
if [[ "$SMOKE_DETACH" == "true" ]]; then
  ...
  echo "    Smoke base URL: $OFFWATCH_PUBLIC_URL"
  echo "    Smoke admin credentials: $SMOKE_ADMIN_EMAIL / $SMOKE_ADMIN_PASSWORD"
