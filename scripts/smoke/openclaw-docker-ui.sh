...
OFFWATCH_HOST_PORT="${OFFWATCH_HOST_PORT:-3100}"
OFFWATCH_HOST_FROM_CONTAINER="${OFFWATCH_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.offwatch-openclaw.override.yml"
...
detect_offwatch_base_url() {
  ...
  for candidate in "$OFFWATCH_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${OFFWATCH_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${OFFWATCH_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
offwatch_base_url="$(detect_offwatch_base_url || true)"
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${OFFWATCH_HOST_PORT}
  (Do not use http://127.0.0.1:${OFFWATCH_HOST_PORT} inside the container.)
