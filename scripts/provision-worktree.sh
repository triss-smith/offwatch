...
base_cwd="${OFFWATCH_WORKSPACE_BASE_CWD:?OFFWATCH_WORKSPACE_BASE_CWD is required}"
worktree_cwd="${OFFWATCH_WORKSPACE_CWD:?OFFWATCH_WORKSPACE_CWD is required}"
offwatch_home="${OFFWATCH_HOME:-$HOME/.offwatch}"
offwatch_instance_id="${OFFWATCH_INSTANCE_ID:-default}"
offwatch_dir="$worktree_cwd/.offwatch"
worktree_config_path="$offwatch_dir/config.json"
worktree_env_path="$offwatch_dir/.env"
worktree_name="${OFFWATCH_WORKSPACE_BRANCH:-$(basename "$worktree_cwd")}"
...
source_config_path="${OFFWATCH_CONFIG:-}"
if [[ -z "$source_config_path" && ( -e "$base_cwd/.offwatch/config.json" || -L "$base_cwd/.offwatch/config.json" ) ]]; then
  source_config_path="$base_cwd/.offwatch/config.json"
fi
if [[ -z "$source_config_path" ]]; then
  source_config_path="$offwatch_home/instances/$offwatch_instance_id/config.json"
fi
source_env_path="$(dirname "$source_config_path")/.env"

mkdir -p "$offwatch_dir"

run_offwatchai_command() {
  local command_args=("$@")
  if command -v pnpm >/dev/null 2>&1 && pnpm offwatchai --help >/dev/null 2>&1; then
    pnpm offwatchai "${command_args[@]}"
    return 0
  fi
  ...
  if command -v offwatchai >/dev/null 2>&1; then
    offwatchai "${command_args[@]}"
    return 0
  fi
...
write_fallback_worktree_config() {
  ...
  OFFWATCH_DIR="$offwatch_dir" \
  SOURCE_CONFIG_PATH="$source_config_path" \
  SOURCE_ENV_PATH="$source_env_path" \
  OFFWATCH_WORKTREES_DIR="${OFFWATCH_WORKTREES_DIR:-}" \
  node <<'EOF'
...
async function main() {
  ...
  const offwatchDir = process.env.OFFWATCH_DIR;
  const sourceConfigPath = process.env.SOURCE_CONFIG_PATH;
  const sourceEnvPath = process.env.SOURCE_ENV_PATH;
  const worktreeHome = path.resolve(expandHomePrefix(nonEmpty(process.env.OFFWATCH_WORKTREES_DIR) ?? "~/.offwatch-worktrees"));
  const instanceId = sanitizeInstanceId(worktreeName);
  const instanceRoot = path.resolve(worktreeHome, "instances", instanceId);
  const configPath = path.resolve(offwatchDir, "config.json");
  const envPath = path.resolve(offwatchDir, ".env");
  ...
  const targetConfig = {
    ...
    storage: {
      ...
      s3: {
        bucket: sourceConfig?.storage?.s3?.bucket ?? "offwatch",
  ...
  const inlineMasterKey = nonEmpty(sourceEnvEntries.OFFWATCH_SECRETS_MASTER_KEY);
  if (inlineMasterKey) {
    ...
    const sourceKeyFilePath = nonEmpty(sourceEnvEntries.OFFWATCH_SECRETS_MASTER_KEY_FILE)
      ? resolveRuntimeLikePath(sourceEnvEntries.OFFWATCH_SECRETS_MASTER_KEY_FILE, sourceConfigPath)
  ...
  const envLines = [
    "OFFWATCH_HOME=" + JSON.stringify(worktreeHome),
    "OFFWATCH_INSTANCE_ID=" + JSON.stringify(instanceId),
    "OFFWATCH_CONFIG=" + JSON.stringify(configPath),
    "OFFWATCH_CONTEXT=" + JSON.stringify(path.resolve(worktreeHome, "context.json")),
    "OFFWATCH_IN_WORKTREE=true",
    "OFFWATCH_WORKTREE_NAME=" + JSON.stringify(worktreeName),
  ];

  const agentJwtSecret = nonEmpty(sourceEnvEntries.OFFWATCH_AGENT_JWT_SECRET);
  if (agentJwtSecret) {
    envLines.push("OFFWATCH_AGENT_JWT_SECRET=" + JSON.stringify(agentJwtSecret));
  }
...
if ! run_isolated_worktree_init; then
  echo "offwatchai CLI not available in this workspace; writing isolated fallback config without DB seeding." >&2
  write_fallback_worktree_config
fi
...
if [[ -f "$worktree_cwd/package.json" && -f "$worktree_cwd/pnpm-lock.yaml" ]]; then
  ...
  if [[ "$needs_install" -eq 1 ]]; then
    backup_suffix=".offwatch-backup-${BASHPID:-$$}"
    moved_symlink_paths=()
