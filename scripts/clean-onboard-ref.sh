...
Environment overrides:
  ...
  OFFWATCH_HOST=127.0.0.1    Host passed to the onboarded server
  OFFWATCH_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/offwatch-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/offwatch-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export OFFWATCH_HOST="${OFFWATCH_HOST:-127.0.0.1}"
export OFFWATCH_PORT="${OFFWATCH_PORT:-3100}"
export OFFWATCH_OPEN_ON_LISTEN="${OFFWATCH_OPEN_ON_LISTEN:-false}"
...
echo "OFFWATCH_HOST: $OFFWATCH_HOST"
echo "OFFWATCH_PORT: $OFFWATCH_PORT"
...
env \
  ...
  HOST="$OFFWATCH_HOST" \
  PORT="$OFFWATCH_PORT" \
  OFFWATCH_OPEN_ON_LISTEN="$OFFWATCH_OPEN_ON_LISTEN" \
  pnpm offwatchai onboard --yes --data-dir "$PC_DATA"
