...
if (tailscaleAuth) {
  ...
  env.HOST = "0.0.0.0";
  console.log("[offwatch] dev mode: authenticated/private (tailscale-friendly) on 0.0.0.0");
} else {
...
async function maybePreflightMigrations(options = {}) {
  ...
  if (!shouldApply) {
    if (exitOnDecline) {
      process.stderr.write(
        `[offwatch] Pending migrations detected (${formatPendingMigrationSummary(pendingMigrations)}). ` +
