...
if (worktreeEnvBootstrap.missingEnv) {
  console.error(
    `[offwatch] linked git worktree at ${repoRoot} is missing ${path.relative(repoRoot, worktreeEnvBootstrap.envPath)}. Run \`offwatchai worktree init\` in this worktree before \`pnpm dev\`.`,
  );
  process.exit(1);
}
...
const devServerStatusFilePath = path.join(repoRoot, ".offwatch", "dev-server-status.json");
...
const ignoredRelativePaths = new Set([
  ".offwatch/dev-server-status.json",
]);
...
for (let index = 0; index < cliArgs.length; index += 1) {
  ...
  if (arg === "--bind") {
    ...
    if (!value || value.startsWith("--") || !BIND_MODES.includes(value as BindMode)) {
      console.error(`[offwatch] invalid --bind value. Use one of: ${BIND_MODES.join(", ")}`);
      process.exit(1);
    }
  ...
  if (arg === "--bind-host") {
    ...
    if (!value || value.startsWith("--")) {
      console.error("[offwatch] --bind-host requires a value");
      process.exit(1);
    }
    bindHost = value;
    index += 1;
...
if (bindMode === "custom" && !bindHost) {
  console.error("[offwatch] --bind custom requires --bind-host <host>");
  process.exit(1);
}

const env: NodeJS.ProcessEnv = {
  ...process.env,
  OFFWATCH_UI_DEV_MIDDLEWARE: "true",
};

if (mode === "dev") {
  env.OFFWATCH_DEV_SERVER_STATUS_FILE = devServerStatusFilePath;
  env.OFFWATCH_MIGRATION_AUTO_APPLY ??= "true";
}

if (mode === "watch") {
  env.OFFWATCH_MIGRATION_PROMPT ??= "never";
  env.OFFWATCH_MIGRATION_AUTO_APPLY ??= "true";
}
...
if (tailscaleAuth || bindMode) {
  ...
  if (tailscaleAuth) {
    console.log("[offwatch] note: --tailscale-auth/--authenticated-private are legacy aliases for --bind lan");
  }
  env.OFFWATCH_BIND = effectiveBind;
  if (bindHost) {
    env.OFFWATCH_BIND_HOST = bindHost;
  } else {
    delete env.OFFWATCH_BIND_HOST;
  }
  if (effectiveBind === "loopback" && !tailscaleAuth) {
    delete env.OFFWATCH_DEPLOYMENT_MODE;
    delete env.OFFWATCH_DEPLOYMENT_EXPOSURE;
    delete env.OFFWATCH_AUTH_BASE_URL_MODE;
    console.log("[offwatch] dev mode: local_trusted (bind=loopback)");
  } else {
    env.OFFWATCH_DEPLOYMENT_MODE = "authenticated";
    env.OFFWATCH_DEPLOYMENT_EXPOSURE = "private";
    env.OFFWATCH_AUTH_BASE_URL_MODE = "auto";
    console.log(
      `[offwatch] dev mode: authenticated/private (bind=${effectiveBind}${bindHost ? `:${bindHost}` : ""})`,
    );
  }
} else {
  delete env.OFFWATCH_BIND;
  delete env.OFFWATCH_BIND_HOST;
  delete env.OFFWATCH_DEPLOYMENT_MODE;
  delete env.OFFWATCH_DEPLOYMENT_EXPOSURE;
  delete env.OFFWATCH_AUTH_BASE_URL_MODE;
  console.log("[offwatch] dev mode: local_trusted (default)");
}
...
if (existingRunner) {
  console.log(
    `[offwatch] ${devService.serviceName} already running (pid ${existingRunner.pid}${typeof existingRunner.metadata?.childPid === "number" ? `, child ${existingRunner.metadata.childPid}` : ""})`,
  );
  process.exit(0);
}
...
async function getMigrationStatusPayload() {
  const status = await runPnpm(
    ["--filter", "@offwatchai/db", "exec", "tsx", "src/migration-status.ts", "--json"],
    { env },
  );
  if (status.code !== 0) {
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        `[offwatch] Command failed with code ${status.code}: pnpm --filter @offwatchai/db exec tsx src/migration-status.ts --json\n`,
    );
  ...
  try {
    ...
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        "[offwatch] migration-status returned invalid JSON payload\n",
    );
...
async function maybePreflightMigrations(options: { interactive?: boolean; autoApply?: boolean; exitOnDecline?: boolean } = {}) {
  ...
  const autoApply = options.autoApply ?? env.OFFWATCH_MIGRATION_AUTO_APPLY === "true";
  ...
  if (!shouldApply) {
    if (exitOnDecline) {
      process.stderr.write(
        `[offwatch] Pending migrations detected (${formatPendingMigrationSummary(pendingMigrations)}). Refusing to start watch mode against a stale schema.\n`,
      );
      process.exit(1);
    }
    return;
...
async function buildPluginSdk() {
  console.log("[offwatch] building plugin sdk...");
  const result = await runPnpm(
    ["--filter", "@offwatchai/plugin-sdk", "build"],
    { stdio: "inherit" },
  );
  if (result.signal) {
  ...
  if (result.code !== 0) {
    console.error("[offwatch] plugin sdk build failed");
...
async function startServerChild() {
  ...
  child = spawn(
    pnpmBin,
    ["--filter", "@offwatchai/server", serverScript, ...forwardedArgs],
