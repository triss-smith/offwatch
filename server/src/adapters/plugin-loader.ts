...
function extractUiParserSource(
  ...
  const contractVersion = pkg.offwatch?.adapterUiParser;
  if (contractVersion) {
    ...
    logger.info(
      { packageName },
      "Adapter has ./ui-parser export but no offwatch.adapterUiParser version — loading anyway (future versions may require it)",
    );
  }
