...
import { getAdapterSessionManagement } from "@offwatch/adapter-utils";
import {
...
import { agentConfigurationDoc as claudeAgentConfigurationDoc, models as claudeModels } from "@offwatch/adapter-claude-local";
import {
...
import { agentConfigurationDoc as codexAgentConfigurationDoc, models as codexModels } from "@offwatch/adapter-codex-local";
import {
...
import { agentConfigurationDoc as cursorAgentConfigurationDoc, models as cursorModels } from "@offwatch/adapter-cursor-local";
import {
...
import { agentConfigurationDoc as geminiAgentConfigurationDoc, models as geminiModels } from "@offwatch/adapter-gemini-local";
import {
...
} from "@offwatch/adapter-opencode-local";
import {
  execute as openclawGatewayExecute,
  testEnvironment as openclawGatewayTestEnvironment,
} from "@offwatch/adapter-openclaw-gateway/server";
import {
  agentConfigurationDoc as openclawGatewayAgentConfigurationDoc,
  models as openclawGatewayModels,
} from "@offwatch/adapter-openclaw-gateway";
...
} from "@offwatch/adapter-pi-local";
import {
...
const externalAdaptersReady: Promise<void> = (async () => {
  try {
    ...
    for (const externalAdapter of externalAdapters) {
      ...
      if (overriding) {
        console.log(
          `[offwatch] External adapter "${externalAdapter.type}" overrides built-in adapter`,
        );
    ...
    console.error("[offwatch] Failed to load external adapters:", err);
  }
})();
...
export function setOverridePaused(type: string, paused: boolean): boolean {
  ...
  if (paused && !wasPaused) {
    ...
    console.log(`[offwatch] Override paused for "${type}" — builtin adapter restored`);
    return true;
  }
  if (!paused && wasPaused) {
    pausedOverrides.delete(type);
    console.log(`[offwatch] Override resumed for "${type}" — external adapter active`);
    return true;
  }
  return false;
}
