const FAVICON_BLOCK_START = "<!-- OFFWATCH_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- OFFWATCH_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- OFFWATCH_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- OFFWATCH_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.OFFWATCH_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.OFFWATCH_WORKTREE_NAME) ?? nonEmpty(env.OFFWATCH_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.OFFWATCH_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="offwatch-worktree-enabled" content="true" />',
    `<meta name="offwatch-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="offwatch-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="offwatch-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
