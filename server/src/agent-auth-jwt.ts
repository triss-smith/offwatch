...
function jwtConfig() {
  const secret = process.env.OFFWATCH_AGENT_JWT_SECRET?.trim() || process.env.BETTER_AUTH_SECRET?.trim();
  if (!secret) return null;

  return {
    secret,
    ttlSeconds: parseNumber(process.env.OFFWATCH_AGENT_JWT_TTL_SECONDS, 60 * 60 * 48),
    issuer: process.env.OFFWATCH_AGENT_JWT_ISSUER ?? "offwatch",
    audience: process.env.OFFWATCH_AGENT_JWT_AUDIENCE ?? "offwatch-api",
  };
}
