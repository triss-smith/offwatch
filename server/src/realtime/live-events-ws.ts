...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@offwatch/db";
import type { DeploymentMode } from "@offwatch/shared";
...
interface IncomingMessageWithContext extends IncomingMessage {
  offwatchUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).offwatchUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.offwatchUpgradeContext = context;
