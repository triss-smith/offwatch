...
import type { OffwatchConfig } from "@offwatch/shared";
...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by Offwatch worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.OFFWATCH_IN_WORKTREE !== "true") return null;

  const configPath = resolveOffwatchConfigPath(overrideConfigPath);
  const envPath = resolveOffwatchEnvPath(configPath);
  const worktreeRoot = path.resolve(path.dirname(configPath), "..");
  const worktreeName = nonEmpty(env.OFFWATCH_WORKTREE_NAME) ?? path.basename(worktreeRoot);
  const instanceId = nonEmpty(env.OFFWATCH_INSTANCE_ID) ?? sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(env.OFFWATCH_HOME) ??
      nonEmpty(env.OFFWATCH_WORKTREES_DIR) ??
      "~/.offwatch-worktrees",
  );
...
function writeConfigFile(configPath: string, config: OffwatchConfig): void {
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".offwatch", "config.json");
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as OffwatchConfig;
...
function buildIsolatedWorktreeConfig(
  ...
  const databasePort =
    config.database.mode === "embedded-postgres"
      ...
      : undefined;
  const nextConfig: OffwatchConfig = {
    ...config,
    database: {
      ...config.database,
      ...(config.database.mode === "embedded-postgres"
...
): { config: OffwatchConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.OFFWATCH_HOME = context.homeDir;
  process.env.OFFWATCH_INSTANCE_ID = context.instanceId;
  process.env.OFFWATCH_CONFIG = context.configPath;
  process.env.OFFWATCH_CONTEXT = context.contextPath;
  process.env.OFFWATCH_WORKTREE_NAME = context.worktreeName;

  let repairedConfig = false;
  if (fs.existsSync(context.configPath)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as OffwatchConfig;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    OFFWATCH_HOME: context.homeDir,
    OFFWATCH_INSTANCE_ID: context.instanceId,
    OFFWATCH_CONFIG: context.configPath,
    OFFWATCH_CONTEXT: context.contextPath,
    OFFWATCH_IN_WORKTREE: "true",
    OFFWATCH_WORKTREE_NAME: context.worktreeName,
  };
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: OffwatchConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as OffwatchConfig;
  } catch {
    return;
  }
