...
import { claudeConfigDir, parseClaudeStreamJson } from "@offwatch/adapter-claude-local/server";
import { codexHomeDir, parseCodexJsonl } from "@offwatch/adapter-codex-local/server";
import { parseOpenCodeJsonl } from "@offwatch/adapter-opencode-local/server";
import {
...
import { resolveHomeAwarePath, resolveOffwatchInstanceRoot } from "../home-paths.js";
...
const FEEDBACK_SCHEMA_VERSION = "offwatch-feedback-envelope-v2";
const FEEDBACK_BUNDLE_VERSION = "offwatch-feedback-bundle-v2";
const FEEDBACK_PAYLOAD_VERSION = "offwatch-feedback-v1";
const FEEDBACK_DESTINATION = "offwatch_labs_feedback_v1";
...
function captureStatusFromFiles(files: FeedbackTraceBundleFile[]): FeedbackTraceBundleCaptureStatus {
  ...
  const hasAdapterFiles = files.some((file) =>
    file.source !== "offwatch_run" &&
    file.source !== "offwatch_run_events" &&
    file.source !== "offwatch_run_log",
  );
...
async function buildOpenCodeTraceFiles(input: {
  ...
  const opencodeRoot = resolveHomeAwarePath(
    process.env.OFFWATCH_OPENCODE_STORAGE_DIR ?? "~/.local/share/opencode",
  );
...
async function resolveFeedbackTarget(
  ...
  if (targetType === "issue_comment") {
    ...
    if (!targetComment || targetComment.issueId !== issue.id || targetComment.workspaceId !== issue.workspaceId) {
  ...
  if (targetType === "issue_document_revision") {
    ...
    if (!targetRevision || targetRevision.workspaceId !== issue.workspaceId) {
      throw notFound("Feedback target not found");
    }
...
async function listIssueContextItems(
  ...
  const [commentRows, revisionRows] = await Promise.all([
    db
      ...
      .from(issueComments)
      .where(and(eq(issueComments.workspaceId, issue.workspaceId), eq(issueComments.issueId, issue.id))),
    db
      .select({
      ...
      .innerJoin(documents, eq(documentRevisions.documentId, documents.id))
      .innerJoin(issueDocuments, eq(issueDocuments.documentId, documents.id))
      .where(and(eq(documentRevisions.workspaceId, issue.workspaceId), eq(issueDocuments.issueId, issue.id))),
  ]);
...
async function buildAgentContext(
  ...
  const desiredSkillRefs = uniqueNonEmpty(readOffwatchSkillSyncPreference(adapterConfig).desiredSkills).slice(0, MAX_SKILLS);
  ...
  const runCosts = run
    ? await db
      ...
      .from(costEvents)
      .where(and(eq(costEvents.workspaceId, workspaceId), eq(costEvents.heartbeatRunId, run.id)))
    : [];
...
async function buildPayloadArtifacts(
  ...
  const [issueContext, agentContext] = await Promise.all([
    ...
    buildAgentContext(db, input.issue.workspaceId, input.target.authorAgentId, input.target.createdByRunId, state),
  ]);
...
async function buildFeedbackTraceBundleFromRow(
  ...
  let offwatchRun: Record<string, unknown> | null = null;
  ...
  if (!sourceRunId) {
    ...
    if (!run || run.workspaceId !== row.workspaceId) {
      ...
      offwatchRun = sanitizeFeedbackValue(
        {
          id: run.id,
      ...
      files.push(makeBundleFile({
        path: "offwatch/run.json",
        contentType: "application/json",
        source: "offwatch_run",
        contents: `${JSON.stringify(offwatchRun, null, 2)}\n`,
      }));

      const sanitizedEvents = sanitizeFeedbackValue(
        events,
        state,
        "bundle.offwatchRun.events",
        MAX_TRACE_FILE_CHARS,
      );
      files.push(makeBundleFile({
        path: "offwatch/run-events.json",
        contentType: "application/json",
        source: "offwatch_run_events",
        contents: `${JSON.stringify(sanitizedEvents, null, 2)}\n`,
      }));

      if (logText) {
        files.push(makeBundleFile({
          path: "offwatch/run-log.ndjson",
          contentType: "application/x-ndjson",
          source: "offwatch_run_log",
          contents: `${sanitizeFeedbackText(logText, state, "bundle.offwatchRun.log", MAX_TRACE_FILE_CHARS)}\n`,
        }));
      } else {
...
export function feedbackService(db: Db, options: FeedbackServiceOptions = {}) {
  return {
    ...
    listFeedbackTraces: async (input: {
      ...
      const filters = [eq(feedbackExports.workspaceId, input.workspaceId)];
    ...
    flushPendingFeedbackTraces: async (input?: {
      ...
      if (!shareClient) {
        ...
        if (input?.workspaceId) {
          filters.push(eq(feedbackExports.workspaceId, input.workspaceId));
        }
        if (input?.traceId) {
