 ...
 */
import { pluginManifestV1Schema } from "@offwatch/shared";
import type { OffwatchPluginManifestV1 } from "@offwatch/shared";
import { PLUGIN_API_VERSION } from "@offwatch/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): OffwatchPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as OffwatchPluginManifestV1,
        };
      }
