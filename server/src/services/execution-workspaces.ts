...
async function inspectGitCloseReadiness(workspace: ExecutionWorkspace): Promise<{
  ...
  if (!workspacePath) {
    warnings.push("Workspace has no local path, so Offwatch cannot inspect git status before close.");
    return { git: null, warnings };
  }

  if (!(await pathExists(workspacePath))) {
    warnings.push(`Workspace path "${workspacePath}" does not exist, so Offwatch cannot inspect git status before close.`);
    return {
      git: {
        repoRoot: null,
        workspacePath,
...
export function executionWorkspaceService(db: Db) {
  return {
    list: async (workspaceId: string, filters?: {
      ...
      const conditions = [eq(executionWorkspaces.workspaceId, workspaceId)];
      ...
      const runtimeServicesByWorkspaceId = await loadEffectiveRuntimeServicesByExecutionWorkspace(db, workspaceId, rows);
      return rows.map((row) =>
    ...
    getById: async (id: string) => {
      ...
      if (!row) return null;
      const runtimeServicesByWorkspaceId = await loadEffectiveRuntimeServicesByExecutionWorkspace(db, row.workspaceId, [row]);
    ...
    getCloseReadiness: async (id: string): Promise<ExecutionWorkspaceCloseReadiness | null> => {
      ...
      const runtimeServicesByWorkspaceId = await loadEffectiveRuntimeServicesByExecutionWorkspace(db, workspace.workspaceId, [workspace]);
      ...
      const linkedIssues = await db
        ...
        .from(issues)
        .where(and(eq(issues.workspaceId, workspace.workspaceId), eq(issues.executionWorkspaceId, workspace.id)));
      ...
      const projectWorkspace = workspace.projectWorkspaceId
        ? await db
            ...
            .where(
              and(
                eq(projectWorkspaces.workspaceId, workspace.workspaceId),
      ...
      const projectPolicy = workspace.projectId
        ? await db
            ...
            .from(projects)
            .where(and(eq(projects.id, workspace.projectId), eq(projects.workspaceId, workspace.workspaceId)))
      ...
      if (executionWorkspace.providerType === "git_worktree" && workspacePath) {
        plannedActions.push({
          ...
          description: `Offwatch will run git worktree cleanup for ${workspacePath}.`,
      ...
      if (git?.createdByRuntime && executionWorkspace.branchName) {
        plannedActions.push({
          ...
          description: "Offwatch will try to delete the runtime-created branch after removing the worktree.",
      ...
      if (executionWorkspace.providerType === "local_fs" && git?.createdByRuntime && workspacePath) {
        ...
        if (containsProjectWorkspace) {
          warnings.push(`Offwatch will archive this workspace but keep "${workspacePath}" because it contains the project workspace.`);
        } else {
          plannedActions.push({
            kind: "remove_local_directory",
            label: "Remove runtime-created directory",
            description: `Offwatch will remove the runtime-created directory at ${workspacePath}.`,
