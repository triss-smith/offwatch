 ...
 *    (`~/.offwatch/plugins/`) and `node_modules` for packages matching
 *    the `offwatch-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published Offwatch plugins.
 * Packages matching this pattern are considered Offwatch plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "offwatch-plugin-";
...
export interface DiscoveredPlugin {
  ...
  manifest: OffwatchPluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.offwatch/plugins/ local directory
  | "npm"               // npm packages matching offwatch-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: OffwatchPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the offwatch-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "offwatch-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: OffwatchPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover Offwatch plugins installed as npm packages in the current
   * Node.js environment matching the "offwatch-plugin-*" naming convention.
   *
   ...
   * the "offwatchPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a Offwatch plugin.
   * Throws if the package is a Offwatch plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<OffwatchPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: OffwatchPluginManifestV1;
    newManifest: OffwatchPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the Offwatch plugin naming convention.
 * Accepts both the "offwatch-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @offwatch/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "offwatchPlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const offwatchPlugin = pkgJson["offwatchPlugin"];
  if (
    offwatchPlugin !== null &&
    typeof offwatchPlugin === "object" &&
    !Array.isArray(offwatchPlugin)
  ) {
    const manifestRelPath = (offwatchPlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: OffwatchPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumOffwatchVersion;
}
 ...
 *   packageName: "offwatch-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: OffwatchPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as OffwatchPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a Offwatch plugin (no manifest found).`,
      );
    }
  ...
  async function loadManifestFromPath(
    manifestPath: string,
  ): Promise<OffwatchPluginManifestV1> {
    let raw: unknown;
   ...
   * if the package is not a Offwatch plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasOffwatchPlugin = "offwatchPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasOffwatchPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<OffwatchPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: OffwatchPluginManifestV1;
      } | null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @offwatch/shared exports). Run those workers through
