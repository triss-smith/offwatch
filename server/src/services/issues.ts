...
import type { IssueRelationIssueSummary } from "@offwatch/shared";
import { extractAgentMentionIds, extractProjectMentionIds, isUuidLike } from "@offwatch/shared";
...
type IssueCreateInput = Omit<typeof issues.$inferInsert, "workspaceId"> & {
  labelIds?: string[];
...
function touchedByUserCondition(workspaceId: string, userId: string) {
  return sql<boolean>`
    (
      ...
      OR EXISTS (
        ...
        WHERE ${issueReadStates.issueId} = ${issues.id}
          AND ${issueReadStates.workspaceId} = ${workspaceId}
      ...
      OR EXISTS (
        ...
        WHERE ${issueComments.issueId} = ${issues.id}
          AND ${issueComments.workspaceId} = ${workspaceId}
...
function participatedByAgentCondition(workspaceId: string, agentId: string) {
  return sql<boolean>`
    (
      ...
      OR EXISTS (
        SELECT 1
        FROM ${activityLog}
        WHERE ${activityLog.workspaceId} = ${workspaceId}
...
function myLastCommentAtExpr(workspaceId: string, userId: string) {
...
function myLastReadAtExpr(workspaceId: string, userId: string) {
  return sql<Date | null>`
    (
...
function myLastTouchAtExpr(workspaceId: string, userId: string) {
  const myLastCommentAt = myLastCommentAtExpr(workspaceId, userId);
  const myLastReadAt = myLastReadAtExpr(workspaceId, userId);
  return sql<Date | null>`
    GREATEST(
...
function lastExternalCommentAtExpr(workspaceId: string, userId: string) {
  return sql<Date | null>`
    (
      SELECT MAX(${issueComments.createdAt})
      FROM ${issueComments}
...
function issueLastActivityAtExpr(workspaceId: string, userId: string) {
  const lastExternalCommentAt = lastExternalCommentAtExpr(workspaceId, userId);
  const myLastTouchAt = myLastTouchAtExpr(workspaceId, userId);
  return sql<Date>`
    GREATEST(
...
function issueLatestCommentAtExpr(workspaceId: string) {
  return sql<Date | null>`
    (
      SELECT MAX(${issueComments.createdAt})
      FROM ${issueComments}
...
function issueLatestLogAtExpr(workspaceId: string) {
  return sql<Date | null>`
    (
...
function issueCanonicalLastActivityAtExpr(workspaceId: string) {
  const latestCommentAt = issueLatestCommentAtExpr(workspaceId);
  const latestLogAt = issueLatestLogAtExpr(workspaceId);
  return sql<Date>`
    GREATEST(
      ${issues.updatedAt},
...
function unreadForUserCondition(workspaceId: string, userId: string) {
  const touchedCondition = touchedByUserCondition(workspaceId, userId);
  const myLastTouchAt = myLastTouchAtExpr(workspaceId, userId);
  return sql<boolean>`
    (
      ${touchedCondition}
...
function inboxVisibleForUserCondition(workspaceId: string, userId: string) {
  const issueLastActivityAt = issueLastActivityAtExpr(workspaceId, userId);
  return sql<boolean>`
    NOT EXISTS (
      SELECT 1
      FROM ${issueInboxArchives}
      WHERE ${issueInboxArchives.issueId} = ${issues.id}
        AND ${issueInboxArchives.workspaceId} = ${workspaceId}
...
export function issueService(db: Db) {
  ...
  async function assertAssignableAgent(workspaceId: string, agentId: string) {
  ...
  async function assertAssignableUser(workspaceId: string, userId: string) {
  ...
  async function assertValidProjectWorkspace(
    ...
    const workspace = await dbOrTx
      .select({
        ...
        workspaceId: projectWorkspaces.workspaceId,
    ...
    if (workspace.workspaceId !== workspaceId) throw unprocessable("Project workspace must belong to same company");
  ...
  async function assertValidExecutionWorkspace(
    ...
    const workspace = await dbOrTx
      .select({
        ...
        workspaceId: executionWorkspaces.workspaceId,
    ...
    if (workspace.workspaceId !== workspaceId) throw unprocessable("Execution workspace must belong to same company");
    if (projectId && workspace.projectId !== projectId) {
      throw unprocessable("Execution workspace must belong to the selected project");
    }
  }

  async function assertValidLabelIds(workspaceId: string, labelIds: string[], dbOrTx: any = db) {
  ...
  async function syncIssueLabels(
    ...
    await assertValidLabelIds(workspaceId, deduped, dbOrTx);
  ...
  async function assertNoBlockingCycles(
    ...
    const rows = await dbOrTx
      ...
      .from(issueRelations)
      .where(and(eq(issueRelations.workspaceId, workspaceId), eq(issueRelations.type, "blocks")));
    ...
    if (deduped.length > 0) {
      ...
      await dbOrTx.execute(
        sql`SELECT ${issues.id} FROM ${issues}
            WHERE ${and(eq(issues.workspaceId, workspaceId), inArray(issues.id, lockedIssueIds))}
            ORDER BY ${issues.id}
            FOR UPDATE`,
      );
      const relatedIssues = await dbOrTx
        .select({ id: issues.id })
        .from(issues)
        .where(and(eq(issues.workspaceId, workspaceId), inArray(issues.id, deduped)));
      if (relatedIssues.length !== deduped.length) {
        throw unprocessable("Blocked-by issues must belong to the same company");
      }
      await assertNoBlockingCycles(workspaceId, issueId, deduped, dbOrTx);
    }
  ...
  return {
    list: async (workspaceId: string, filters?: IssueFilters) => {
      ...
      if (filters?.participantAgentId) {
        conditions.push(participatedByAgentCondition(workspaceId, filters.participantAgentId));
      }
      if (filters?.assigneeUserId) {
        conditions.push(eq(issues.assigneeUserId, filters.assigneeUserId));
      }
      if (touchedByUserId) {
        conditions.push(touchedByUserCondition(workspaceId, touchedByUserId));
      }
      if (inboxArchivedByUserId) {
        conditions.push(inboxVisibleForUserCondition(workspaceId, inboxArchivedByUserId));
      }
      if (unreadForUserId) {
        conditions.push(unreadForUserCondition(workspaceId, unreadForUserId));
      }
      ...
      if (filters?.labelId) {
        const labeledIssueIds = await db
          ...
          .from(issueLabels)
          .where(and(eq(issueLabels.workspaceId, workspaceId), eq(issueLabels.labelId, filters.labelId)));
      ...
      const searchOrder = sql<number>`
        ...
        END
      `;
      const canonicalLastActivityAt = issueCanonicalLastActivityAtExpr(workspaceId);
      const baseQuery = db
        .select()
        .from(issues)
      ...
      const [statsRows, readRows, lastActivityRows] = await Promise.all([
        ...
        contextUserId
          ? db
            ...
            .where(
              and(
                eq(issueReadStates.workspaceId, workspaceId),
    ...
    countUnreadTouchedByUser: async (workspaceId: string, userId: string, status?: string) => {
      const conditions = [
        eq(issues.workspaceId, workspaceId),
        isNull(issues.hiddenAt),
        unreadForUserCondition(workspaceId, userId),
    ...
    markRead: async (workspaceId: string, issueId: string, userId: string, readAt: Date = new Date()) => {
      const now = new Date();
      const [row] = await db
        ...
        .onConflictDoUpdate({
          target: [issueReadStates.workspaceId, issueReadStates.issueId, issueReadStates.userId],
          set: {
            lastReadAt: readAt,
            updatedAt: now,
          },
    ...
    markUnread: async (workspaceId: string, issueId: string, userId: string) => {
    ...
    archiveInbox: async (workspaceId: string, issueId: string, userId: string, archivedAt: Date = new Date()) => {
      const now = new Date();
      const [row] = await db
        ...
        .onConflictDoUpdate({
          target: [issueInboxArchives.workspaceId, issueInboxArchives.issueId, issueInboxArchives.userId],
          set: {
            archivedAt,
            updatedAt: now,
          },
    ...
    unarchiveInbox: async (workspaceId: string, issueId: string, userId: string) => {
      const [row] = await db
        .delete(issueInboxArchives)
        .where(
          and(
            eq(issueInboxArchives.workspaceId, workspaceId),
    ...
    getRelationSummaries: async (issueId: string) => {
      ...
      const relations = await getIssueRelationSummaryMap(issue.workspaceId, [issueId], db);
    ...
    listWakeableBlockedDependents: async (blockerIssueId: string) => {
      ...
      const candidates = await db
        ...
        .where(
          and(
            eq(issueRelations.workspaceId, blockerIssue.workspaceId),
            eq(issueRelations.type, "blocks"),
    ...
    getWakeableParentAfterChildCompletion: async (parentIssueId: string) => {
      ...
      const children = await db
        ...
        .from(issues)
        .where(and(eq(issues.workspaceId, parent.workspaceId), eq(issues.parentId, parentIssueId)));
    ...
    create: async (
      ...
      if (data.assigneeAgentId) {
        await assertAssignableAgent(workspaceId, data.assigneeAgentId);
      }
      if (data.assigneeUserId) {
        await assertAssignableUser(workspaceId, data.assigneeUserId);
      }
      if (data.status === "in_progress" && !data.assigneeAgentId && !data.assigneeUserId) {
        throw unprocessable("in_progress issues require an assignee");
      }
      return db.transaction(async (tx) => {
        const defaultCompanyGoal = await getDefaultCompanyGoal(tx, workspaceId);
        const projectGoalId = await getProjectDefaultGoalId(tx, workspaceId, issueData.projectId);
        ...
        if (workspaceInheritanceIssueId) {
          const workspaceSource = await getWorkspaceInheritanceIssue(tx, workspaceId, workspaceInheritanceIssueId);
        ...
        if (
          ...
          const project = await tx
            ...
            .from(projects)
            .where(and(eq(projects.id, issueData.projectId), eq(projects.workspaceId, workspaceId)))
            .then((rows) => rows[0] ?? null);
          executionWorkspaceSettings =
            defaultIssueExecutionWorkspaceSettingsForProject(
              gateProjectExecutionWorkspacePolicy(
        ...
        if (!projectWorkspaceId && issueData.projectId) {
          ...
          if (!projectWorkspaceId) {
            projectWorkspaceId = await tx
              ...
              .from(projectWorkspaces)
              .where(and(eq(projectWorkspaces.projectId, issueData.projectId), eq(projectWorkspaces.workspaceId, workspaceId)))
              .orderBy(desc(projectWorkspaces.isPrimary), asc(projectWorkspaces.createdAt), asc(projectWorkspaces.id))
              .then((rows) => rows[0]?.id ?? null);
          }
        }
        if (projectWorkspaceId) {
          await assertValidProjectWorkspace(workspaceId, issueData.projectId, projectWorkspaceId, tx);
        }
        if (executionWorkspaceId) {
          await assertValidExecutionWorkspace(workspaceId, issueData.projectId, executionWorkspaceId, tx);
        }
        ...
        const [company] = await tx
          .update(workspaces)
          .set({
            issueCounter: sql`greatest(${workspaces.issueCounter}, ${currentMax}) + 1`,
          })
          .where(eq(workspaces.id, workspaceId))
          .returning({ issueCounter: workspaces.issueCounter, issuePrefix: workspaces.issuePrefix });
        ...
        if (inputLabelIds) {
          await syncIssueLabels(issue.id, workspaceId, inputLabelIds, tx);
        }
    ...
    update: async (
      ...
      if (issueData.assigneeAgentId) {
        await assertAssignableAgent(existing.workspaceId, issueData.assigneeAgentId);
      }
      if (issueData.assigneeUserId) {
        await assertAssignableUser(existing.workspaceId, issueData.assigneeUserId);
      }
      ...
      if (nextProjectWorkspaceId) {
        await assertValidProjectWorkspace(existing.workspaceId, nextProjectId, nextProjectWorkspaceId);
      }
      if (nextExecutionWorkspaceId) {
        await assertValidExecutionWorkspace(existing.workspaceId, nextProjectId, nextExecutionWorkspaceId);
      }
      ...
      const runUpdate = async (tx: any) => {
        const defaultCompanyGoal = await getDefaultCompanyGoal(tx, existing.workspaceId);
        const [currentProjectGoalId, nextProjectGoalId] = await Promise.all([
          getProjectDefaultGoalId(tx, existing.workspaceId, existing.projectId),
        ...
        if (nextLabelIds !== undefined) {
          await syncIssueLabels(updated.id, existing.workspaceId, nextLabelIds, tx);
        }
        if (blockedByIssueIds !== undefined) {
          await syncBlockedByIssueIds(
            updated.id,
    ...
    checkout: async (id: string, agentId: string, expectedStatuses: string[], checkoutRunId: string | null) => {
      ...
      await assertAssignableAgent(issueCompany.workspaceId, agentId);
    ...
    listLabels: (workspaceId: string) =>
      db.select().from(labels).where(eq(labels.workspaceId, workspaceId)).orderBy(asc(labels.name), asc(labels.id)),
    ...
    createLabel: async (workspaceId: string, data: Pick<typeof labels.$inferInsert, "name" | "color">) => {
    ...
    createAttachment: async (input: {
      ...
      if (input.issueCommentId) {
        const comment = await db
          .select({ id: issueComments.id, workspaceId: issueComments.workspaceId, issueId: issueComments.issueId })
          .from(issueComments)
          .where(eq(issueComments.id, input.issueCommentId))
          .then((rows) => rows[0] ?? null);
        if (!comment) throw notFound("Issue comment not found");
        if (comment.workspaceId !== issue.workspaceId || comment.issueId !== issue.id) {
    ...
    findMentionedAgents: async (workspaceId: string, body: string) => {
      const re = /\B@([^\s@,!?.]+)/g;
    ...
    findMentionedProjectIds: async (issueId: string) => {
      ...
      const rows = await db
        ...
        .where(
          and(
            eq(projects.workspaceId, issue.workspaceId),
