...
import type { AdapterRuntimeServiceReport } from "@offwatch/adapter-utils";
import type { Db } from "@offwatch/db";
import { executionWorkspaces, projectWorkspaces, workspaceRuntimeServices } from "@offwatch/db";
...
export async function listWorkspaceRuntimeServicesForProjectWorkspaces(
  ...
  const rows = await db
    ...
    .where(
      and(
        eq(workspaceRuntimeServices.workspaceId, workspaceId),
