/**
 * PluginEventBus — typed in-process event bus for the Offwatch plugin system.
 *
 * Responsibilities:
...
import type { PluginEventType } from "@offwatch/shared";
import type { PluginEvent, EventFilter } from "@offwatch/plugin-sdk";
 ...
 *
 * - `workspaceId` — always resolved from `payload.workspaceId`. Core domain events that
...
function passesFilter(event: PluginEvent, filter: EventFilter | null): boolean {
  ...
  if (filter.workspaceId !== undefined) {
    if (event.workspaceId !== filter.workspaceId) return false;
  }
...
export function createPluginEventBus(): PluginEventBus {
  ...
  function forPlugin(pluginId: string): ScopedPluginEventBus {
    return {
       ...
       */
      async emit(name: string, workspaceId: string, payload: unknown): Promise<PluginEventBusEmitResult> {
        if (!name || name.trim() === "") {
          throw new Error(`Plugin "${pluginId}" must provide a non-empty event name.`);
        }

        if (!workspaceId || workspaceId.trim() === "") {
          throw new Error(`Plugin "${pluginId}" must provide a workspaceId when emitting events.`);
        }
...
export interface ScopedPluginEventBus {
   ...
   */
  emit(name: string, workspaceId: string, payload: unknown): Promise<PluginEventBusEmitResult>;
