...
import { pluginLogs, agentTaskSessions as agentTaskSessionsTable } from "@offwatch/db";
...
export function buildHostServices(
  ...
  const ensureCompanyId = (workspaceId?: string) => {
   ...
   */
  const ensurePluginAvailableForCompany = async (_workspaceId: string) => {};

  const inCompany = <T extends { workspaceId: string | null | undefined }>(
    record: T | null | undefined,
    workspaceId: string,
  ): record is T => Boolean(record && record.workspaceId === workspaceId);

  const requireInCompany = <T extends { workspaceId: string | null | undefined }>(
    entityName: string,
    record: T | null | undefined,
    workspaceId: string,
  ): T => {
    if (!inCompany(record, workspaceId)) {
  ...
  return {
    ...
    events: {
      async emit(params) {
        ...
        await scopedBus.emit(params.name, params.workspaceId, params.payload);
      },
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@offwatch/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {
    ...
    activity: {
      async log(params) {
        const workspaceId = ensureCompanyId(params.workspaceId);
        await ensurePluginAvailableForCompany(workspaceId);
    ...
    projects: {
      async list(params) {
        const workspaceId = ensureCompanyId(params.workspaceId);
        await ensurePluginAvailableForCompany(workspaceId);
        return applyWindow((await projects.list(workspaceId)) as Project[], params);
      },
      async get(params) {
        const workspaceId = ensureCompanyId(params.workspaceId);
        await ensurePluginAvailableForCompany(workspaceId);
        const project = await projects.getById(params.projectId);
        return (inCompany(project, workspaceId) ? project : null) as Project | null;
      },
      async listWorkspaces(params) {
        const workspaceId = ensureCompanyId(params.workspaceId);
        await ensurePluginAvailableForCompany(workspaceId);
        const project = await projects.getById(params.projectId);
        if (!inCompany(project, workspaceId)) return [];
      ...
      async getPrimaryWorkspace(params) {
        ...
        await ensurePluginAvailableForCompany(workspaceId);
        const project = await projects.getById(params.projectId);
        if (!inCompany(project, workspaceId)) return null;
      ...
      async getWorkspaceForIssue(params) {
        ...
        if (!inCompany(issue, workspaceId)) return null;
    ...
    issues: {
      async list(params) {
        const workspaceId = ensureCompanyId(params.workspaceId);
        await ensurePluginAvailableForCompany(workspaceId);
        return applyWindow((await issues.list(workspaceId, params as any)) as Issue[], params);
      },
      async get(params) {
        const workspaceId = ensureCompanyId(params.workspaceId);
        await ensurePluginAvailableForCompany(workspaceId);
        const issue = await issues.getById(params.issueId);
        return (inCompany(issue, workspaceId) ? issue : null) as Issue | null;
      },
      async create(params) {
        const workspaceId = ensureCompanyId(params.workspaceId);
        await ensurePluginAvailableForCompany(workspaceId);
        return (await issues.create(workspaceId, params as any)) as Issue;
      },
      async update(params) {
        const workspaceId = ensureCompanyId(params.workspaceId);
        await ensurePluginAvailableForCompany(workspaceId);
        requireInCompany("Issue", await issues.getById(params.issueId), workspaceId);
        return (await issues.update(params.issueId, params.patch as any)) as Issue;
      },
      async listComments(params) {
        const workspaceId = ensureCompanyId(params.workspaceId);
        await ensurePluginAvailableForCompany(workspaceId);
        if (!inCompany(await issues.getById(params.issueId), workspaceId)) return [];
    ...
    agents: {
      ...
      async get(params) {
        ...
        return (inCompany(agent, workspaceId) ? agent : null) as Agent | null;
      },
      async pause(params) {
        const workspaceId = ensureCompanyId(params.workspaceId);
        await ensurePluginAvailableForCompany(workspaceId);
        const agent = await agents.getById(params.agentId);
        requireInCompany("Agent", agent, workspaceId);
    ...
    goals: {
      async list(params) {
        const workspaceId = ensureCompanyId(params.workspaceId);
        await ensurePluginAvailableForCompany(workspaceId);
        const rows = await goals.list(workspaceId);
        return applyWindow(
          rows.filter((goal) =>
      ...
      async get(params) {
        ...
        return (inCompany(goal, workspaceId) ? goal : null) as Goal | null;
      },
      async create(params) {
        const workspaceId = ensureCompanyId(params.workspaceId);
        await ensurePluginAvailableForCompany(workspaceId);
        return (await goals.create(workspaceId, {
          title: params.title,
      ...
      async update(params) {
        const workspaceId = ensureCompanyId(params.workspaceId);
        await ensurePluginAvailableForCompany(workspaceId);
        requireInCompany("Goal", await goals.getById(params.goalId), workspaceId);
    ...
    agentSessions: {
        ...
        const rows = await db
          ...
          .where(
            and(
              ...
              eq(agentTaskSessionsTable.workspaceId, workspaceId),
      ...
      async sendMessage(params) {
        ...
        if (notifyWorker) {
          ...
          const unsubscribe = subscribeCompanyLiveEvents(workspaceId, (event) => {
