...
import type { DeploymentExposure, DeploymentMode } from "@offwatch/shared";
import {
  forbidden,
  conflict,
  notFound,
...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (
    normalized !== "offwatch" &&
    normalized !== "offwatch-create-agent" &&
    normalized !== "offwatch-create-plugin" &&
...
/** Resolve the Offwatch repo skills directory (built-in / managed skills). */
function resolveOffwatchSkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  const offwatchSkillsDir = resolveOffwatchSkillsDir();

  // Build set of Offwatch-managed skill names
  const offwatchSkillNames = new Set<string>();
  if (offwatchSkillsDir) {
    try {
      for (const entry of fs.readdirSync(offwatchSkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory()) offwatchSkillNames.add(entry.name);
      }
    } catch { /* skip */ }
  }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        name: entry.name,
        description,
        isOffwatchManaged: offwatchSkillNames.has(entry.name),
      });
    }
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.offwatchApiUrl)) {
    const legacyOffwatchApiUrl = nonEmptyTrimmedString(input.offwatchApiUrl);
    if (legacyOffwatchApiUrl) merged.offwatchApiUrl = legacyOffwatchApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    offwatchApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.offwatchApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawOffwatchApiUrl =
    typeof defaults.offwatchApiUrl === "string"
      ? defaults.offwatchApiUrl.trim()
      : "";
  if (rawOffwatchApiUrl) {
    try {
      const parsedOffwatchApiUrl = new URL(rawOffwatchApiUrl);
      if (
        parsedOffwatchApiUrl.protocol !== "http:" &&
        parsedOffwatchApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_offwatch_api_url_protocol",
          level: "warn",
          message: `offwatchApiUrl must use http:// or https:// (got ${parsedOffwatchApiUrl.protocol}).`
        });
      } else {
        normalized.offwatchApiUrl = parsedOffwatchApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_offwatch_api_url_configured",
          level: "info",
          message: `offwatchApiUrl set to ${parsedOffwatchApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_offwatch_api_url_invalid",
        level: "warn",
        message: `Invalid offwatchApiUrl: ${rawOffwatchApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your Offwatch host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "Offwatch is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm offwatchai allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = "/api/skills/offwatch";
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/offwatch-claimed-api-key.json and load OFFWATCH_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: offwatchApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the Offwatch hostname is reachable and allowed via `pnpm offwatchai allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this Offwatch API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/offwatch/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # Offwatch OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    BODY="$(node -e '
      ...
      const body = {
        ...
        agentDefaultsPayload: {
          ...
          offwatchApiUrl: "http://host.docker.internal:3100",
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, Offwatch generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "offwatchApiUrl": "https://offwatch-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in Offwatch before key claim is allowed.
    ...
    - ~/.openclaw/workspace/offwatch-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/offwatch-claimed-api-key.json

    And set the OFFWATCH_API_KEY and OFFWATCH_API_URL in your environment variables as specified here:
    ...
    Then set OFFWATCH_API_KEY and OFFWATCH_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install Offwatch skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your OFFWATCH_API_URL to the top of your skill and note the path to your OFFWATCH_API_URL
    ...
    ${
      ...
      "Ensure Offwatch is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested Offwatch base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.offwatchApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm offwatchai allowed-hostname <host>
      - then restart Offwatch and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@offwatch.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (_req, res) => {
    res.json({
      skills: [
        { name: "offwatch", path: "/api/skills/offwatch" },
        {
          name: "para-memory-files",
          path: "/api/skills/para-memory-files"
        },
        {
          name: "offwatch-create-agent",
          path: "/api/skills/offwatch-create-agent"
        }
      ]
    });
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              offwatchApiUrl: req.body.offwatchApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.offwatchApiUrl &&
          !persistedDefaults.offwatchApiUrl
        ) {
          missingPersistedFields.push("offwatchApiUrl");
        }
