...
import { issues, projects, projectWorkspaces } from "@offwatch/db";
import { updateExecutionWorkspaceSchema } from "@offwatch/shared";
...
export function executionWorkspaceRoutes(db: Db) {
  ...
  router.get("/workspaces/:workspaceId/execution-workspaces", async (req, res) => {
    const workspaceId = req.params.workspaceId as string;
    assertCompanyAccess(req, workspaceId);
    const workspaces = await svc.list(workspaceId, {
  ...
  router.get("/execution-workspaces/:id", async (req, res) => {
    ...
    assertCompanyAccess(req, workspace.workspaceId);
    res.json(workspace);
  });
  ...
  router.post("/execution-workspaces/:id/runtime-services/:action", async (req, res) => {
    ...
    if (!workspaceCwd) {
      res.status(422).json({ error: "Execution workspace needs a local path before Offwatch can manage local runtime services" });
      return;
    }
    ...
    if (req.body.status === "archived" && existing.status !== "archived") {
      ...
      if (existing.mode === "shared_workspace") {
        await db
          ...
          .where(
            and(
              eq(issues.workspaceId, existing.workspaceId),
      ...
      try {
        ...
        const projectPolicy = existing.projectId
          ? await db
              ...
              .from(projects)
              .where(and(eq(projects.id, existing.projectId), eq(projects.workspaceId, existing.workspaceId)))
