...
export function projectRoutes(db: Db) {
  ...
  async function normalizeProjectReference(req: Request, rawId: string) {
    ...
    const workspaceId = await resolveCompanyIdForProjectReference(req);
    if (!workspaceId) return rawId;
    const resolved = await svc.resolveByReference(workspaceId, rawId);
  ...
  router.get("/projects/:id", async (req, res) => {
    ...
    assertCompanyAccess(req, project.workspaceId);
    res.json(project);
  });

  router.post("/workspaces/:workspaceId/projects", validate(createProjectSchema), async (req, res) => {
    const workspaceId = req.params.workspaceId as string;
    assertCompanyAccess(req, workspaceId);
    ...
    const project = await svc.create(workspaceId, projectData);
  ...
  router.patch("/projects/:id", validate(updateProjectSchema), async (req, res) => {
    ...
    if (body.env !== undefined) {
      body.env = await secretsSvc.normalizeEnvBindingsForPersistence(existing.workspaceId, body.env, {
  ...
  router.post("/projects/:id/workspaces/:workspaceId/runtime-services/:action", async (req, res) => {
    ...
    const actor = getActorInfo(req);
    const recorder = workspaceOperations.createRecorder({ workspaceId: project.workspaceId });
