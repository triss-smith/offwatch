...
describe("POST /api/workspaces/:workspaceId/assets/images", () => {
  beforeEach(() => {
    vi.resetModules();
  ...
  it("accepts PNG image uploads and returns an asset path", async () => {
    ...
    const res = await request(app)
      .post("/api/workspaces/company-1/assets/images")
...
describe("POST /api/workspaces/:workspaceId/logo", () => {
  beforeEach(() => {
    vi.resetModules();
    registerServiceMocks();
    createAssetMock.mockReset();
  ...
  it("accepts PNG logo uploads and returns an asset path", async () => {
    ...
    const res = await request(app)
      .post("/api/workspaces/company-1/logo")
      .attach("file", Buffer.from("png"), "logo.png");
