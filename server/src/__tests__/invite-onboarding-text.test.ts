...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("Offwatch OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("Suggested Offwatch base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("offwatchApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.offwatchApiUrl");
    expect(text).toContain("~/.openclaw/workspace/offwatch-claimed-api-key.json");
    expect(text).toContain("OFFWATCH_API_KEY");
