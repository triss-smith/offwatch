...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "offwatch", LOGNAME: "offwatch", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "offwatch" }),
      },
    );

    expect(tokens).toEqual(["offwatch", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "offwatch" }, {
        userInfo: () => ({ username: "offwatch" }),
      });

      expect(tokens).toEqual(["offwatch", "custom-token"]);
    } finally {
