...
describe("agent instructions service", () => {
  const originalOffwatchHome = process.env.OFFWATCH_HOME;
  const originalOffwatchInstanceId = process.env.OFFWATCH_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalOffwatchHome === undefined) delete process.env.OFFWATCH_HOME;
    else process.env.OFFWATCH_HOME = originalOffwatchHome;
    if (originalOffwatchInstanceId === undefined) delete process.env.OFFWATCH_INSTANCE_ID;
    else process.env.OFFWATCH_INSTANCE_ID = originalOffwatchInstanceId;
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const offwatchHome = await makeTempDir("offwatch-agent-instructions-home-");
    const externalRoot = await makeTempDir("offwatch-agent-instructions-external-");
    cleanupDirs.add(offwatchHome);
    cleanupDirs.add(externalRoot);
    process.env.OFFWATCH_HOME = offwatchHome;
    process.env.OFFWATCH_INSTANCE_ID = "test-instance";
  ...
  it("creates the target entry file when switching to a new external root", async () => {
    ...
    const externalRoot = await makeTempDir("offwatch-agent-instructions-new-external-");
    cleanupDirs.add(offwatchHome);
    cleanupDirs.add(externalRoot);
    process.env.OFFWATCH_HOME = offwatchHome;
    process.env.OFFWATCH_INSTANCE_ID = "test-instance";
  ...
  it("filters junk files, dependency bundles, and python caches from bundle listings and exports", async () => {
    const externalRoot = await makeTempDir("offwatch-agent-instructions-ignore-");
    cleanupDirs.add(externalRoot);
  ...
  it("recovers a managed bundle from disk when bundle config metadata is missing", async () => {
    const offwatchHome = await makeTempDir("offwatch-agent-instructions-recover-");
    cleanupDirs.add(offwatchHome);
    process.env.OFFWATCH_HOME = offwatchHome;
    process.env.OFFWATCH_INSTANCE_ID = "test-instance";
  ...
  it("prefers the managed bundle on disk when managed metadata points at a stale root", async () => {
    const offwatchHome = await makeTempDir("offwatch-agent-instructions-stale-managed-");
    const staleRoot = await makeTempDir("offwatch-agent-instructions-stale-root-");
    cleanupDirs.add(offwatchHome);
  ...
  it("heals stale managed metadata when writing bundle files", async () => {
    const offwatchHome = await makeTempDir("offwatch-agent-instructions-heal-write-");
    const staleRoot = await makeTempDir("offwatch-agent-instructions-heal-write-stale-");
    cleanupDirs.add(offwatchHome);
    cleanupDirs.add(staleRoot);
    process.env.OFFWATCH_HOME = offwatchHome;
    process.env.OFFWATCH_INSTANCE_ID = "test-instance";
  ...
  it("heals stale managed metadata when deleting bundle files", async () => {
    const offwatchHome = await makeTempDir("offwatch-agent-instructions-heal-delete-");
    const staleRoot = await makeTempDir("offwatch-agent-instructions-heal-delete-stale-");
    cleanupDirs.add(offwatchHome);
    cleanupDirs.add(staleRoot);
    process.env.OFFWATCH_HOME = offwatchHome;
    process.env.OFFWATCH_INSTANCE_ID = "test-instance";
  ...
  it("recovers the managed bundle when stale root metadata is present but mode is missing", async () => {
    const offwatchHome = await makeTempDir("offwatch-agent-instructions-partial-managed-");
    const staleRoot = await makeTempDir("offwatch-agent-instructions-partial-root-");
    cleanupDirs.add(offwatchHome);
    cleanupDirs.add(staleRoot);
    process.env.OFFWATCH_HOME = offwatchHome;
    process.env.OFFWATCH_INSTANCE_ID = "test-instance";
