...
import { writeOffwatchSkillSyncPreference } from "@offwatch/adapter-utils/server-utils";
import {
  agents,
...
async function startTempDatabase() {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "offwatch-feedback-service-"));
  ...
  const adminConnectionString = `postgres://offwatch:offwatch@127.0.0.1:${port}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "offwatch");
  const connectionString = `postgres://offwatch:offwatch@127.0.0.1:${port}/offwatch`;
...
describe("feedbackService.saveIssueVote", () => {
  ...
  async function seedIssueWithRichAgentComment() {
    ...
    const instructionsDir = fs.mkdtempSync(path.join(os.tmpdir(), "offwatch-feedback-instructions-"));
    ...
    await db.insert(agents).values({
      ...
      status: "active",
      adapterType: "codex_local",
      adapterConfig: writeOffwatchSkillSyncPreference(
        {
          model: "gpt-5.4",
        ...
        ["offwatchai/offwatch/offwatch", "octo/research/public-skill"],
      ),
      runtimeConfig: {
        heartbeat: {
          enabled: true,
  ...
  it("builds a detailed sanitized shared bundle with issue and agent context", async () => {
    ...
    expect(trace?.schemaVersion).toBe("offwatch-feedback-envelope-v2");
    expect(trace?.bundleVersion).toBe("offwatch-feedback-bundle-v2");
  ...
  it("keeps earlier local votes local when a later vote enables sharing", async () => {
    ...
    expect(localTrace?.payloadVersion).toBe("offwatch-feedback-v1");
    ...
    expect(sharedTrace?.payloadVersion).toBe("offwatch-feedback-v1");
  });

  it("captures Claude project session artifacts as full traces", async () => {
    const claudeRoot = fs.mkdtempSync(path.join(os.tmpdir(), "offwatch-feedback-claude-"));
  ...
  it("captures OpenCode message and part files as full traces", async () => {
    const opencodeRoot = fs.mkdtempSync(path.join(os.tmpdir(), "offwatch-feedback-opencode-"));
    ...
    vi.stubEnv("OFFWATCH_OPENCODE_STORAGE_DIR", opencodeRoot);
    const uploadTraceBundle = vi.fn().mockResolvedValue({ objectKey: "feedback-traces/test.json" });
    const flushingSvc = feedbackService(db, {
      shareClient: {
        uploadTraceBundle,
