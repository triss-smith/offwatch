...
import { execute, testEnvironment } from "@offwatch/adapter-openclaw-gateway/server";
import {
  buildOpenClawGatewayConfig,
  parseOpenClawGatewayStdoutLine,
} from "@offwatch/adapter-openclaw-gateway/ui";
import type { AdapterExecutionContext } from "@offwatch/adapter-utils";
...
describe("openclaw gateway adapter execute", () => {
  it("runs connect -> agent -> agent.wait and forwards wake payload", async () => {
    ...
    try {
      ...
      expect(payload?.sessionKey).toBe("offwatch:issue:issue-123");
      expect(String(payload?.message ?? "")).toContain("wake now");
      expect(String(payload?.message ?? "")).toContain("OFFWATCH_RUN_ID=run-123");
      expect(String(payload?.message ?? "")).toContain("OFFWATCH_TASK_ID=task-123");
      expect(String(payload?.message ?? "")).toContain("## Offwatch Wake Payload");
      ...
      expect(payload?.offwatch).toMatchObject({
        wake: {
          latestCommentId: "comment-2",
          commentIds: ["comment-1", "comment-2"],
        },
