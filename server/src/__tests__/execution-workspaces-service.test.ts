...
async function createTempRepo() {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "offwatch-execution-workspace-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.name", "Offwatch Test"]);
  await runGit(repoRoot, ["config", "user.email", "test@offwatch.local"]);
...
describeEmbeddedPostgres("executionWorkspaceService.getCloseReadiness", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("offwatch-execution-workspaces-service-");
  ...
  it("warns about dirty and unmerged git worktrees and reports cleanup actions", async () => {
    ...
    const worktreePath = path.join(path.dirname(repoRoot), `offwatch-worktree-${randomUUID()}`);
    tempDirs.add(worktreePath);

    await runGit(repoRoot, ["branch", "offwatch-close-check"]);
    await runGit(repoRoot, ["worktree", "add", worktreePath, "offwatch-close-check"]);
    ...
    await db.insert(executionWorkspaces).values({
      ...
      branchName: "offwatch-close-check",
      baseRef: "main",
      metadata: {
  ...
  it("shows inherited shared project runtime services on shared execution workspaces without duplicating old history", async () => {
    ...
    const reuseKey = `project_workspace:${projectWorkspaceId}:offwatch-dev`;
    ...
    await db.insert(projectWorkspaces).values({
      ...
      metadata: {
        runtimeConfig: {
          ...
          workspaceRuntime: {
            services: [{ name: "offwatch-dev", command: "pnpm dev" }],
          },
        },
      },
    });
