...
describe("worktree config repair", () => {
  it("repairs legacy repo-local worktree config and env files into an isolated instance", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "offwatch-worktree-repair-"));
    const worktreeRoot = path.join(tempRoot, "PAP-884-ai-commits-component");
    const offwatchDir = path.join(worktreeRoot, ".offwatch");
    const configPath = path.join(offwatchDir, "config.json");
    const envPath = path.join(offwatchDir, ".env");
    const sharedRoot = path.join(tempRoot, ".offwatch", "instances", "default");
    const isolatedHome = path.join(tempRoot, ".offwatch-worktrees");

    await fs.mkdir(offwatchDir, { recursive: true });
    await fs.writeFile(configPath, JSON.stringify(buildLegacyConfig(sharedRoot), null, 2) + "\n", "utf8");
    await fs.writeFile(
      envPath,
      [
        "# Offwatch environment variables",
        "OFFWATCH_IN_WORKTREE=true",
        "OFFWATCH_WORKTREE_NAME=PAP-884-ai-commits-component",
        "OFFWATCH_AGENT_JWT_SECRET=shared-secret",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.env.OFFWATCH_WORKTREE_NAME = "PAP-884-ai-commits-component";
    process.env.OFFWATCH_WORKTREES_DIR = isolatedHome;
    delete process.env.OFFWATCH_HOME;
    delete process.env.OFFWATCH_INSTANCE_ID;
    delete process.env.OFFWATCH_CONFIG;
    delete process.env.OFFWATCH_CONTEXT;
    ...
    expect(repairedEnv).toContain(`OFFWATCH_HOME=${JSON.stringify(isolatedHome)}`);
    expect(repairedEnv).toContain('OFFWATCH_INSTANCE_ID="pap-884-ai-commits-component"');
    expect(repairedEnv).toContain(`OFFWATCH_CONFIG=${JSON.stringify(await fs.realpath(configPath))}`);
    expect(repairedEnv).toContain(`OFFWATCH_CONTEXT=${JSON.stringify(path.join(isolatedHome, "context.json"))}`);
    expect(repairedEnv).toContain('OFFWATCH_AGENT_JWT_SECRET="shared-secret"');
    expect(process.env.OFFWATCH_HOME).toBe(isolatedHome);
    expect(process.env.OFFWATCH_INSTANCE_ID).toBe("pap-884-ai-commits-component");
  });

  it("avoids sibling worktree ports when repairing legacy configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "offwatch-worktree-repair-ports-"));
    ...
    await fs.writeFile(
      ...
      [
        "# Offwatch environment variables",
        "OFFWATCH_IN_WORKTREE=true",
        "OFFWATCH_WORKTREE_NAME=PAP-880-thumbs-capture-for-evals-feature",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.chdir(worktreeRoot);
    process.env.OFFWATCH_IN_WORKTREE = "true";
    process.env.OFFWATCH_WORKTREE_NAME = "PAP-880-thumbs-capture-for-evals-feature";
    process.env.OFFWATCH_WORKTREES_DIR = isolatedHome;
  ...
  it("rebalances duplicate ports for already isolated worktree configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "offwatch-worktree-rebalance-"));
    const isolatedHome = path.join(tempRoot, ".offwatch-worktrees");
    const repoWorktreesRoot = path.join(tempRoot, "repo", ".offwatch", "worktrees");
    ...
    const offwatchDir = path.join(currentWorktreeRoot, ".offwatch");
    const configPath = path.join(offwatchDir, "config.json");
    const envPath = path.join(offwatchDir, ".env");
    const currentInstanceRoot = path.join(isolatedHome, "instances", "pap-884-ai-commits-component");
    const siblingConfigPath = path.join(siblingWorktreeRoot, ".offwatch", "config.json");
  ...
  it("persists runtime-selected worktree ports back into config", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "offwatch-worktree-ports-"));
    ...
    process.chdir(worktreeRoot);
    process.env.OFFWATCH_IN_WORKTREE = "true";
    process.env.OFFWATCH_WORKTREE_NAME = "PAP-878-create-a-mine-tab-in-inbox";
    process.env.OFFWATCH_HOME = isolatedHome;
    process.env.OFFWATCH_INSTANCE_ID = "pap-878-create-a-mine-tab-in-inbox";
