...
describe("agent local JWT", () => {
  const secretEnv = "OFFWATCH_AGENT_JWT_SECRET";
  const betterAuthSecretEnv = "BETTER_AUTH_SECRET";
  const ttlEnv = "OFFWATCH_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "OFFWATCH_AGENT_JWT_ISSUER";
  const audienceEnv = "OFFWATCH_AGENT_JWT_AUDIENCE";
  ...
  it("falls back to BETTER_AUTH_SECRET when OFFWATCH_AGENT_JWT_SECRET is absent", () => {
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "offwatch";
    process.env[audienceEnv] = "offwatch-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
