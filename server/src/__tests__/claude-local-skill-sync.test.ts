...
describe("claude local skill sync", () => {
  ...
  const createAgentKey = "offwatchai/offwatch/offwatch-create-agent";
  ...
  it("defaults to mounting all built-in Offwatch skills when no explicit selection exists", async () => {
    ...
    expect(snapshot.entries.find((entry) => entry.key === offwatchKey)?.required).toBe(true);
  ...
  it("normalizes legacy flat Offwatch skill refs to canonical keys", async () => {
    const snapshot = await listClaudeSkills({
      agentId: "agent-3",
      workspaceId: "workspace-1",
      adapterType: "claude_local",
  ...
  it("shows host-level user-installed Claude skills as read-only external entries", async () => {
    const home = await makeTempDir("offwatch-claude-user-skills-");
