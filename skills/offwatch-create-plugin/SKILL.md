---
name: offwatch-create-plugin
description: >
  Create new Offwatch plugins with the current alpha SDK/runtime. Use when
...
# Create a Offwatch Plugin

Use this skill when the task is to create, scaffold, or document a Offwatch plugin.
...
```bash
pnpm --filter @offwatchai/create-offwatch-plugin build
node packages/plugins/create-offwatch-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the Offwatch repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.offwatch-sdk/`:

```bash
pnpm --filter @offwatchai/create-offwatch-plugin build
node packages/plugins/create-offwatch-plugin/dist/index.js @acme/plugin-name \
  --output /absolute/path/to/plugin-repos \
  --sdk-path /absolute/path/to/offwatch/packages/plugins/sdk
```
...
- is installed into Offwatch from an absolute local path during development
