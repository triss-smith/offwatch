...
import { ApiConnectionError, ApiRequestError, OffwatchApiClient } from "../client/http.js";

describe("OffwatchApiClient", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new OffwatchApiClient({
    ...
    expect(headers["x-offwatch-run-id"]).toBe("run-abc");
