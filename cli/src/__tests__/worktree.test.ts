...
function buildSourceConfig(): OffwatchConfig {
  return {
    $meta: {
      version: 1,
...
describe("worktree helpers", () => {
  ...
  it("resolves worktree:make target paths under the user home directory", () => {
    expect(resolveWorktreeMakeTargetPath("offwatch-pr-432")).toBe(
      path.resolve(os.homedir(), "offwatch-pr-432"),
    );
  });

  it("rejects worktree:make names that are not safe directory/branch names", () => {
    expect(() => resolveWorktreeMakeTargetPath("offwatch/pr-432")).toThrow(
  ...
  it("rewrites loopback auth URLs to the new port only", () => {
    ...
    expect(rewriteLocalUrlPort("https://offwatch.example", 3110)).toBe("https://offwatch.example");
  });

  it("builds isolated config and env paths for a worktree", () => {
    const paths = resolveWorktreeLocalPaths({
      cwd: "/tmp/offwatch-feature",
      homeDir: "/tmp/offwatch-worktrees",
    ...
    expect(config.database.embeddedPostgresDataDir).toBe(
      path.resolve("/tmp/offwatch-worktrees", "instances", "feature-worktree-support", "db"),
    );
    ...
    expect(config.storage.localDisk.baseDir).toBe(
      path.resolve("/tmp/offwatch-worktrees", "instances", "feature-worktree-support", "data", "storage"),
    );
    ...
    expect(env.PAPERCLIP_HOME).toBe(path.resolve("/tmp/offwatch-worktrees"));
  ...
  it("copies the source local_encrypted secrets key into the seeded worktree instance", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "offwatch-worktree-secrets-"));
  ...
  it("persists the current agent jwt secret into the worktree env file", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "offwatch-worktree-jwt-"));
    ...
    try {
      ...
      await worktreeInitCommand({
        ...
        home: path.join(tempRoot, ".offwatch-worktrees"),
      });

      const envPath = path.join(repoRoot, ".offwatch", ".env");
  ...
  it("avoids ports already claimed by sibling worktree instance configs", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "offwatch-worktree-claimed-ports-"));
    const repoRoot = path.join(tempRoot, "repo");
    const homeDir = path.join(tempRoot, ".offwatch-worktrees");
    ...
    try {
      ...
      const config = JSON.parse(fs.readFileSync(path.join(repoRoot, ".offwatch", "config.json"), "utf8"));
  ...
  it("defaults the seed source config to the current repo-local Offwatch config", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "offwatch-worktree-source-config-"));
    const repoRoot = path.join(tempRoot, "repo");
    const localConfigPath = path.join(repoRoot, ".offwatch", "config.json");
    const originalCwd = process.cwd();
    const originalOffwatchConfig = process.env.PAPERCLIP_CONFIG;
    ...
    try {
      ...
      process.chdir(originalCwd);
      if (originalOffwatchConfig === undefined) {
        delete process.env.PAPERCLIP_CONFIG;
      } else {
        process.env.PAPERCLIP_CONFIG = originalOffwatchConfig;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("preserves the source config path across worktree:make cwd changes", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "offwatch-worktree-source-override-"));
    const sourceConfigPath = path.join(tempRoot, "source", "config.json");
  ...
  it("derives worktree reseed target paths from the adjacent env file", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "offwatch-worktree-reseed-target-"));
    const worktreeRoot = path.join(tempRoot, "repo");
    const configPath = path.join(worktreeRoot, ".offwatch", "config.json");
    const envPath = path.join(worktreeRoot, ".offwatch", ".env");
    ...
    try {
      ...
      fs.writeFileSync(
        envPath,
        [
          "PAPERCLIP_HOME=/tmp/offwatch-worktrees",
  ...
  it("rejects reseed targets without worktree env metadata", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "offwatch-worktree-reseed-target-missing-"));
    const worktreeRoot = path.join(tempRoot, "repo");
    const configPath = path.join(worktreeRoot, ".offwatch", "config.json");
    ...
    try {
      fs.mkdirSync(path.dirname(configPath), { recursive: true });
      fs.writeFileSync(configPath, JSON.stringify(buildSourceConfig()), "utf8");
      fs.writeFileSync(path.join(worktreeRoot, ".offwatch", ".env"), "", "utf8");

      expect(() =>
        resolveWorktreeReseedTargetPaths({
          configPath,
          rootPath: worktreeRoot,
        })).toThrow("does not look like a worktree-local Offwatch instance");
    } finally {
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("reseed preserves the current worktree ports, instance id, and branding", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "offwatch-worktree-reseed-"));
    const repoRoot = path.join(tempRoot, "repo");
    ...
    const sourcePaths = resolveWorktreeLocalPaths({
      cwd: sourceRoot,
      homeDir: path.join(tempRoot, ".offwatch-source"),
      instanceId: "default",
    });
    const originalCwd = process.cwd();
    const originalOffwatchConfig = process.env.PAPERCLIP_CONFIG;
  ...
  it("restores the current worktree config and instance data if reseed fails", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "offwatch-worktree-reseed-rollback-"));
    const repoRoot = path.join(tempRoot, "repo");
    const sourceRoot = path.join(tempRoot, "source");
    const homeDir = path.join(tempRoot, ".offwatch-worktrees");
  ...
  it("rebinds same-repo workspace paths onto the current worktree root", () => {
    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/offwatch",
        targetRepoRoot: "/Users/example/offwatch-pr-432",
        workspaceCwd: "/Users/example/offwatch",
      }),
    ).toBe("/Users/example/offwatch-pr-432");

    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/offwatch",
        targetRepoRoot: "/Users/example/offwatch-pr-432",
        workspaceCwd: "/Users/example/offwatch/packages/db",
      }),
    ).toBe("/Users/example/offwatch-pr-432/packages/db");
  });
  ...
  it("copies shared git hooks into a linked worktree git dir", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "offwatch-worktree-hooks-"));
    const repoRoot = path.join(tempRoot, "repo");
  ...
  it("creates and initializes a worktree from the top-level worktree:make command", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "offwatch-worktree-make-"));
    const repoRoot = path.join(tempRoot, "repo");
    const fakeHome = path.join(tempRoot, "home");
    const worktreePath = path.join(fakeHome, "offwatch-make-test");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      await worktreeMakeCommand("offwatch-make-test", {
        seed: false,
        home: path.join(tempRoot, ".offwatch-worktrees"),
      });

      expect(fs.existsSync(path.join(worktreePath, ".git"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".offwatch", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".offwatch", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
...
describeEmbeddedPostgres("pauseSeededScheduledRoutines", () => {
  it("pauses only routines with enabled schedule triggers", async () => {
    const tempDb = await startEmbeddedPostgresTestDatabase("offwatch-worktree-routines-");
