...
export function agentJwtSecretCheck(configPath?: string): CheckResult {
  if (readAgentJwtSecretFromEnv(configPath)) {
    return {
      ...
      status: "pass",
      message: "OFFWATCH_AGENT_JWT_SECRET is set in environment",
    };
  }
  ...
  if (fileSecret) {
    return {
      name: "Agent JWT secret",
      status: "warn",
      message: `OFFWATCH_AGENT_JWT_SECRET is present in ${envPath} but not loaded into environment`,
  ...
  return {
    name: "Agent JWT secret",
    status: "fail",
    message: `OFFWATCH_AGENT_JWT_SECRET missing from environment and ${envPath}`,
    canRepair: true,
    repair: () => {
      ensureAgentJwtSecret(configPath);
    },
    repairHint: `Run with --repair to create ${envPath} containing OFFWATCH_AGENT_JWT_SECRET`,
  };
}
