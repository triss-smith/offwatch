...
import type { OffwatchConfig } from "./schema.js";
...
export function ensureLocalSecretsKeyFile(
  config: Pick<OffwatchConfig, "secrets">,
  configPath?: string,
  ...
  const envMasterKey = process.env.OFFWATCH_SECRETS_MASTER_KEY;
  if (envMasterKey && envMasterKey.trim().length > 0) {
    return { status: "skipped_env", path: null };
  }

  const keyFileOverride = process.env.OFFWATCH_SECRETS_MASTER_KEY_FILE;
