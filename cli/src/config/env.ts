...
const JWT_SECRET_ENV_KEY = "OFFWATCH_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    ...
    "# Generated by Offwatch CLI commands",
...
export function resolveOffwatchEnvFile(configPath?: string): string {
...
export function loadOffwatchEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeOffwatchEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readOffwatchEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeOffwatchEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeOffwatchEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readOffwatchEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeOffwatchEnvEntries(next, filePath);
  return next;
}
