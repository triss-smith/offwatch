...
async function openConfiguredDb(configPath: string): Promise<{
  ...
  try {
    if (config.database.mode === "embedded-postgres") {
      ...
      const adminConnectionString = `postgres://offwatch:offwatch@127.0.0.1:${embeddedHandle.port}/postgres`;
      await ensurePostgresDatabase(adminConnectionString, "offwatch");
      const connectionString = `postgres://offwatch:offwatch@127.0.0.1:${embeddedHandle.port}/offwatch`;
