...
export async function doctor(opts: {
  ...
  p.intro(pc.bgCyan(pc.black(" offwatch doctor ")));
  ...
  try {
    ...
    const readResult: CheckResult = {
      ...
      canRepair: false,
      repairHint: "Run `offwatchai configure --section database` or `offwatchai onboard`",
    };
...
async function runRepairableCheck(input: {
  ...
  loadOffwatchEnvFile(input.configPath);
