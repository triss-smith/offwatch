...
import { ensureAgentJwtSecret, loadOffwatchEnvFile, mergeOffwatchEnvEntries, readOffwatchEnvEntries, resolveOffwatchEnvFile } from "../config/env.js";
...
const WORKTREE_NAME_PREFIX = "offwatch-";
...
function createConfiguredStorageFromOffwatchConfig(config: OffwatchConfig): ConfiguredStorage {
...
function openConfiguredStorage(configPath: string): ConfiguredStorage {
  ...
  return createConfiguredStorageFromOffwatchConfig(config);
}
...
function resolveRepoManagedWorktreesRoot(cwd: string): string | null {
  ...
  const marker = `${path.sep}.offwatch${path.sep}worktrees${path.sep}`;
  const index = normalized.indexOf(marker);
  if (index === -1) return null;
  const repoRoot = normalized.slice(0, index);
  return path.resolve(repoRoot, ".offwatch", "worktrees");
}
...
function collectClaimedWorktreePorts(homeDir: string, currentInstanceId: string, cwd: string): {
  ...
  if (repoManagedWorktreesRoot && existsSync(repoManagedWorktreesRoot)) {
    for (const entry of readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const configPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".offwatch", "config.json");
      if (existsSync(configPath)) {
        configPaths.add(configPath);
      }
    }
...
export function resolveSourceConfigPath(opts: WorktreeInitOptions): string {
  ...
  const sourceHome = path.resolve(expandHomePrefix(opts.fromDataDir ?? "~/.offwatch"));
...
export function resolveWorktreeReseedTargetPaths(input: {
  ...
  const envEntries = readOffwatchEnvEntries(resolveOffwatchEnvFile(input.configPath));
  ...
  if (!homeDir || !instanceId) {
    throw new Error(
      `Target config ${input.configPath} does not look like a worktree-local Offwatch instance. Expected PAPERCLIP_HOME and PAPERCLIP_INSTANCE_ID in the adjacent .env.`,
    );
  }
...
function resolveSourceConnectionString(config: OffwatchConfig, envEntries: Record<string, string>, portOverride?: number): string {
  ...
  return `postgres://offwatch:offwatch@127.0.0.1:${port}/offwatch`;
}

export function copySeededSecretsKey(input: {
  sourceConfigPath: string;
  sourceConfig: OffwatchConfig;
...
async function seedWorktreeDatabase(input: {
  sourceConfigPath: string;
  sourceConfig: OffwatchConfig;
  targetConfig: OffwatchConfig;
  targetPaths: WorktreeLocalPaths;
  instanceId: string;
  seedMode: WorktreeSeedMode;
}): Promise<SeedWorktreeDatabaseResult> {
  const seedPlan = resolveWorktreeSeedPlan(input.seedMode);
  const sourceEnvFile = resolveOffwatchEnvFile(input.sourceConfigPath);
  const sourceEnvEntries = readOffwatchEnvEntries(sourceEnvFile);
  ...
  try {
    if (input.sourceConfig.database.mode === "embedded-postgres") {
      ...
      const sourceAdminConnectionString = `postgres://offwatch:offwatch@127.0.0.1:${sourceHandle.port}/postgres`;
      await ensurePostgresDatabase(sourceAdminConnectionString, "offwatch");
    }
    ...
    const adminConnectionString = `postgres://offwatch:offwatch@127.0.0.1:${targetHandle.port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "offwatch");
    const targetConnectionString = `postgres://offwatch:offwatch@127.0.0.1:${targetHandle.port}/offwatch`;
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  const sourceEnvEntries = readOffwatchEnvEntries(resolveOffwatchEnvFile(sourceConfigPath));
  ...
  loadOffwatchEnvFile(paths.configPath);
  ...
  p.outro(
    pc.green(
      `Worktree ready. Run Offwatch inside this repo and the CLI/server will use ${paths.instanceId} automatically.`,
    ),
  );
}
...
export async function worktreeInitCommand(opts: WorktreeInitOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" offwatchai worktree init ")));
  await runWorktreeInit(opts);
}

export async function worktreeMakeCommand(nameArg: string, opts: WorktreeMakeOptions): Promise<void> {
  printOffwatchCliBanner();
  p.intro(pc.bgCyan(pc.black(" offwatchai worktree:make ")));
...
function toMergeSourceChoices(cwd: string): MergeSourceChoice[] {
  ...
  return parseGitWorktreeList(cwd).map((entry) => {
    ...
    return {
      ...
      branch: entry.branch,
      branchLabel,
      hasOffwatchConfig: existsSync(path.resolve(worktreePath, ".offwatch", "config.json")),
...
export async function worktreeCleanupCommand(nameArg: string, opts: WorktreeCleanupOptions): Promise<void> {
  printOffwatchCliBanner();
  p.intro(pc.bgCyan(pc.black(" offwatchai worktree:cleanup ")));
...
export async function worktreeEnvCommand(opts: WorktreeEnvOptions): Promise<void> {
  ...
  const envPath = resolveOffwatchEnvFile(configPath);
  const envEntries = readOffwatchEnvEntries(envPath);
  const out = {
...
function resolveAttachmentLookupStorages(input: {
  ...
  const orderedConfigPaths = [
    ...
    ...toMergeSourceChoices(process.cwd())
      .filter((choice) => choice.hasOffwatchConfig)
      .map((choice) => path.resolve(choice.worktree, ".offwatch", "config.json")),
  ];
...
async function openConfiguredDb(configPath: string): Promise<OpenDbHandle> {
  ...
  const envEntries = readOffwatchEnvEntries(resolveOffwatchEnvFile(configPath));
    ...
    if (migrationState.status !== "upToDate") {
      const pending =
        migrationState.reason === "pending-migrations"
          ...
          : "";
      throw new Error(
        `Database for ${configPath} is not up to date.${pending} Run \`pnpm db:migrate\` (or start Offwatch once) before using worktree merge history.`,
      );
    }
...
function resolveRunningEmbeddedPostgresPid(config: OffwatchConfig): number | null {
  if (config.database.mode !== "embedded-postgres") {
    return null;
  }
...
export async function worktreeListCommand(opts: WorktreeListOptions): Promise<void> {
  ...
  for (const choice of choices) {
    const flags = [
      ...
      choice.hasOffwatchConfig ? "offwatch" : "no-offwatch-config",
...
function resolveEndpointFromChoice(choice: MergeSourceChoice): ResolvedWorktreeEndpoint {
  ...
  return {
    ...
    configPath: path.resolve(choice.worktree, ".offwatch", "config.json"),
...
function resolveWorktreeEndpointFromSelector(
  ...
  if (existsSync(directPath)) {
    ...
    const configPath = path.resolve(directPath, ".offwatch", "config.json");
    if (!existsSync(configPath)) {
      throw new Error(`Resolved worktree path ${directPath} does not contain .offwatch/config.json.`);
    }
    return {
      rootPath: directPath,
      configPath,
  ...
  if (!matched.hasOffwatchConfig && !matched.isCurrent) {
    throw new Error(`Resolved worktree "${selector}" does not look like a Offwatch worktree.`);
  }
...
async function promptForSourceEndpoint(excludeWorktreePath?: string): Promise<ResolvedWorktreeEndpoint> {
  ...
  const currentEndpoint = resolveCurrentEndpoint();
  const choices = toMergeSourceChoices(process.cwd())
    .filter((choice) => choice.hasOffwatchConfig || choice.isCurrent)
  ...
  if (choices.length === 0) {
    throw new Error("No Offwatch worktrees were found. Run `offwatchai worktree:list` to inspect the repo worktrees.");
  }
...
export async function worktreeMergeHistoryCommand(sourceArg: string | undefined, opts: WorktreeMergeHistoryOptions): Promise<void> {
  ...
  if (path.resolve(sourceEndpoint.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target Offwatch configs are the same. Choose different --from/--to worktrees.");
  }
...
export async function worktreeReseedCommand(opts: WorktreeReseedOptions): Promise<void> {
  printOffwatchCliBanner();
  p.intro(pc.bgCyan(pc.black(" offwatchai worktree reseed ")));
  ...
  if (path.resolve(source.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target Offwatch configs are the same. Choose different --from/--to values.");
  }
  ...
  if (runningTargetPid && !opts.allowLiveTarget) {
    throw new Error(
      `Target worktree database appears to be running (pid ${runningTargetPid}). Stop Offwatch in ${targetEndpoint.rootPath} before reseeding, or re-run with --allow-live-target if you want to override this guard.`,
    );
  }
  ...
  const confirmed = opts.yes
    ? true
    : await p.confirm({
      message: `Overwrite the isolated Offwatch DB for ${targetEndpoint.label} from ${source.label} using ${seedMode} seed mode?`,
      initialValue: false,
    });
  if (p.isCancel(confirmed) || !confirmed) {
...
export function registerWorktreeCommands(program: Command): void {
  const worktree = program.command("worktree").description("Worktree-local Offwatch instance helpers");

  program
    .command("worktree:make")
    .description("Create ~/NAME as a git worktree, then initialize an isolated Offwatch instance inside it")
    .argument("<name>", "Worktree name — auto-prefixed with offwatch- if needed (created at ~/offwatch-NAME)")
  ...
  worktree
    .command("env")
    .description("Print shell exports for the current worktree-local Offwatch instance")
  ...
  program
    ...
    .description("List git worktrees visible from this repo and whether they look like Offwatch worktrees")
  ...
  worktree
    .command("reseed")
    .description("Re-seed an existing worktree-local instance from another Offwatch instance or worktree")
    .option("--from <worktree>", "Source worktree path, directory name, branch name, or current")
    .option("--to <worktree>", "Target worktree path, directory name, branch name, or current (defaults to current)")
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source PAPERCLIP_HOME used when deriving the source config")
  ...
  program
    ...
    .argument("<name>", "Worktree name — auto-prefixed with offwatch- if needed")
