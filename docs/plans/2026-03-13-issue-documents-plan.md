...
Add first-class **documents** to Offwatch as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by Offwatch workflow/docs.
...
Update the Offwatch agent workflow so planning no longer edits the issue description.

Required changes:

- update `skills/offwatch/SKILL.md`
