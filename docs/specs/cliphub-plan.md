# ClipHub: Marketplace for Offwatch Team Configurations
...
> The "app store" for whole-company AI teams — pre-built Offwatch configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Offwatch-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Offwatch company (agents, projects, governance) |
...
A complete Offwatch company configuration:
...
Single-agent configurations designed to plug into a Offwatch org:
...
Portable skill files that any Offwatch agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Offwatch's skill loading system
...
interface Listing {
  ...
  offwatchVersionMin: string;     // Minimum Offwatch version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Offwatch company
...
| `POST` | `/api/listings/:id/install` | Install to Offwatch company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Offwatch company (or create new)
  → ClipHub API calls Offwatch API to:
  ...
  → Redirect to Offwatch dashboard with new team running
```
...
### 5.3 Creator: Export from Offwatch → Publish

```
Running Offwatch company → "Export as Blueprint" (CLI or UI)
  → Offwatch exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Offwatch brand blue/purple)
...
| Install flow | One-click deploy to Offwatch company |
...
{
  "targetCompanyId": "uuid",         // Existing Offwatch company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `offwatch-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Offwatch
- **Backend**: Node.js API (or extend Offwatch server)
- **Database**: Postgres (can share Offwatch's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Offwatch auth (or OAuth2)

### 9.2 Integration with Offwatch

ClipHub can be:
- **Option A**: A separate app that calls Offwatch's API to install blueprints
- **Option B**: A built-in section of the Offwatch UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Offwatch UI and API.
...
- [ ] Install flow: blueprint → Offwatch company
...
- [ ] "Export from Offwatch" CLI command
