...
- Running a Python script that calls the Offwatch API
...
1. Offwatch spawns the configured command as a child process
2. Standard Offwatch environment variables are injected (`PAPERCLIP_AGENT_ID`, `PAPERCLIP_API_KEY`, etc.)
...
The script can use the injected environment variables to authenticate with the Offwatch API and perform work.
