...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only Offwatch.
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.offwatch.yaml`
...
- Offwatch must not require extra top-level fields for skill validity
- Offwatch-specific extensions must live under `metadata.offwatch` or `metadata.sources`
...
For Offwatch, the preferred fidelity extension is:
...
- issue/task Offwatch-only metadata
...
- Offwatch tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested Offwatch shape:
...
Additional rules for Offwatch exporters:
...
- omit empty and default-valued Offwatch fields when possible
...
- Offwatch exporters should emit `.offwatch.yaml` as a sidecar by default
...
Offwatch can map this spec to its runtime model like this:
...
- Offwatch extension:
  - `.offwatch.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other Offwatch-specific fidelity

Inline Offwatch-only metadata that must live inside a shared markdown file should use:

- `metadata.offwatch`

That keeps the base format broader than Offwatch.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only Offwatch.

## 20. Cutover

Offwatch should cut over to this markdown-first package model as the primary portability format.

`offwatch.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For Offwatch, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
