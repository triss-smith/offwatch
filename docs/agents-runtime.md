...
Audience: Operators setting up and running agents in Offwatch

## 1. What this system does

Agents in Offwatch do not run continuously.  
...
- `hermes_local`: runs your local `hermes` CLI (`hermes-offwatch-adapter`)
...
- `droid_local`: runs your local Factory Droid CLI (`@henkey/droid-offwatch-adapter`)

For local CLI adapters (`claude_local`, `codex_local`, `opencode_local`, `hermes_local`, `droid_local`), Offwatch assumes the CLI is already installed and authenticated on the host machine.
...
Offwatch stores session IDs for resumable adapters.
...
Offwatch pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. Offwatch surfaces this as a warning in environment tests, not a hard error.
