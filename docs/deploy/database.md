...
Offwatch uses PostgreSQL via Drizzle ORM. There are three ways to run the database.
...
1. Creates `~/.offwatch/instances/default/db/` for storage
2. Ensures the `offwatch` database exists
3. Runs migrations automatically
4. Starts serving requests

Data persists across restarts. To reset: `rm -rf ~/.offwatch/instances/default/db`.
...
```sh
cp .env.example .env
# DATABASE_URL=postgres://offwatch:offwatch@localhost:5432/offwatch
```
