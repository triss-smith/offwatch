...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-Offwatch-Signature` + `X-Offwatch-Timestamp` header pair matching the trigger's signing mode.
