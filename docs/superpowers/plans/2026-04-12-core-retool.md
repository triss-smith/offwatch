...
**Goal:** Retool Offwatch Lite from a multi-company AI workforce platform into a personal orchestration tool — rename `companies` → `workspaces`, strip org hierarchy, add issue tracker fields, git worktree isolation, and simplified approvals.
...
**Tech Stack:** PostgreSQL + Drizzle ORM (migrations via `pnpm db:generate` + manual SQL edits for renames), Express.js services and routes, `@offwatchai/shared` for types, React + TanStack Query for UI. Tests use Vitest.

> **Note on naming:** This plan uses "workspace" as the new top-level container name. Offwatch already has `execution_workspaces` and `project_workspaces` — these are sub-concepts and keep their names. Context makes the distinction clear.
...
```bash
pnpm --filter @offwatchai/db typecheck
```
...
```bash
pnpm --filter @offwatchai/shared typecheck
```
...
```bash
pnpm --filter @offwatchai/server typecheck
```
...
```bash
pnpm --filter @offwatchai/ui typecheck
```
