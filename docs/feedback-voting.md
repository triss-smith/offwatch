...
When you rate an agent's response with **Helpful** (thumbs up) or **Needs work** (thumbs down), Offwatch saves your vote locally alongside your running instance. This guide covers what gets stored, how to access it, and how to export it.
...
```bash
pnpm offwatchai feedback report
```
...
```bash
# Installed CLI
offwatchai feedback report

# Point to a different server or company
pnpm offwatchai feedback report --api-base http://127.0.0.1:3000 --company-id <company-id>

# Include raw payload dumps in the report
pnpm offwatchai feedback report --payloads
```
...
```bash
pnpm offwatchai feedback export
```
...
feedback-export-20260331T120000Z/
  ...
  traces/
    PAP-123-e5f6g7h8.json      # Offwatch feedback envelope (one per trace)
  full-traces/
    PAP-123-e5f6g7h8/
...
Exports are full by default. `traces/` keeps the Offwatch envelope, while `full-traces/` contains the richer per-trace bundle plus any recoverable adapter-native files.

```bash
# Custom server and output directory
pnpm offwatchai feedback export --api-base http://127.0.0.1:3000 --company-id <company-id> --out ./my-export
```
...
- Default endpoint: when no feedback export backend URL is configured, Offwatch falls back to `https://telemetry.offwatch.ing`
