...
How to get OpenClaw running in a Docker container for local development and testing the Offwatch OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

Offwatch includes an end-to-end join smoke harness:
...
- writes isolated smoke config under `~/.openclaw-offwatch-smoke/openclaw.json` and Docker `.env`
...
- probes and prints a Offwatch host URL that is reachable from inside OpenClaw Docker
...
- `OPENCLAW_CONFIG_DIR` (default `~/.openclaw-offwatch-smoke`)
...
If your Offwatch deployment is `authenticated`, provide auth context:
...
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host Offwatch server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed Offwatch URL (typically `http://host.docker.internal:3100`).
- If Offwatch rejects the container-visible host with a hostname error, allow it from host:

```bash
pnpm offwatchai allowed-hostname host.docker.internal
```

Then restart Offwatch and rerun the smoke script.
