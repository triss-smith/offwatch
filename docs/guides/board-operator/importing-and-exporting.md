...
Offwatch companies can be exported to portable markdown packages and imported from local directories or GitHub repositories. This lets you share company configurations, duplicate setups, and version-control your agent teams.
...
├── tasks/
│   └── onboarding/TASK.md
└── .offwatch.yaml     # Adapter config, env inputs, routines
```
...
- **.offwatch.yaml** holds Offwatch-specific config (adapter types, env inputs, budgets) as an optional sidecar.
...
```sh
offwatchai company export <company-id> --out ./my-export
```
...
offwatchai company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
offwatchai company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
offwatchai company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
- Adapter type and env input declarations in `.offwatch.yaml`
...
```sh
# From a local folder
offwatchai company import ./my-export

# From a GitHub URL
offwatchai company import https://github.com/org/repo

# From a GitHub subfolder
offwatchai company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
offwatchai company import org/repo
offwatchai company import org/repo/companies/acme
```
...
If `--target` is not specified, Offwatch infers it: if a `--company-id` is provided (or one exists in context), it defaults to `existing`; otherwise `new`.
...
```sh
offwatchai company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
offwatchai company import org/company-templates/engineering-team \
  --target new \
...
```sh
offwatchai company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
offwatchai company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
offwatchai company import ./package \
  --target new \
  --yes \
  --json
```
...
Offwatch supports several GitHub URL formats:
