  ...
  const packageJson = {
    ...
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@offwatch/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@offwatch/plugin-sdk": sdkDependency,
...
import { createPluginBundlerPresets } from "@offwatch/plugin-sdk/bundlers";
...
`,
  ...
  writeFile(
    ...
    `import { definePlugin, runWorker } from "@offwatch/plugin-sdk";
...
`,
  ...
  writeFile(
    ...
    `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@offwatch/plugin-sdk/ui";
...
import { createTestHarness } from "@offwatch/plugin-sdk/testing";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@offwatch/plugin-sdk\` and \`@offwatch/shared\` from a local Paperclip checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.paperclip-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}
...
- \`pnpm build\` uses esbuild presets from \`@offwatch/plugin-sdk/bundlers\`.
