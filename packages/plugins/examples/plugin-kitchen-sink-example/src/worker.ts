...
import {
  ...
  type ToolResult,
  type ToolRunContext,
} from "@offwatch/plugin-sdk";
import type { Goal, Issue } from "@offwatch/shared";
import {
  DEFAULT_CONFIG,
  JOB_KEYS,
  PLUGIN_ID,
...
async function resolveWorkspace(
  ...
  const workspaces = await ctx.projects.listWorkspaces(projectId, workspaceId);
  if (workspaces.length === 0) {
    throw new Error("No workspaces configured for this project");
  }
  if (!projectWorkspaceId) return workspaces[0]!;
  const workspace = workspaces.find((entry) => entry.id === projectWorkspaceId);
  if (!workspace) {
...
async function runCuratedCommand(
  ...
  const workspace = await resolveWorkspace(ctx, workspaceId, projectId, projectWorkspaceId);
...
function getCurrentWorkspaceId(params: Record<string, unknown>): string {
...
async function listIssuesForCompany(ctx: PluginContext, workspaceId: string, limit = 50): Promise<Issue[]> {
  return await ctx.issues.list({ workspaceId, limit, offset: 0 });
}

async function listGoalsForCompany(ctx: PluginContext, workspaceId: string, limit = 50): Promise<Goal[]> {
  return await ctx.goals.list({ workspaceId, limit, offset: 0 });
}
...
async function registerDataHandlers(ctx: PluginContext): Promise<void> {
  ...
  ctx.data.register("overview", async (params) => {
    ...
    const projects = workspaceId ? await ctx.projects.list({ workspaceId, limit: 200, offset: 0 }) : [];
    const issues = workspaceId ? await listIssuesForCompany(ctx, workspaceId, 200) : [];
    const goals = workspaceId ? await listGoalsForCompany(ctx, workspaceId, 200) : [];
    const agents = workspaceId ? await ctx.agents.list({ workspaceId, limit: 200, offset: 0 }) : [];
  ...
  ctx.data.register("projects", async (params) => {
    const workspaceId = getCurrentWorkspaceId(params);
    return await ctx.projects.list({ workspaceId, limit: getListLimit(params), offset: 0 });
  });

  ctx.data.register("issues", async (params) => {
    const workspaceId = getCurrentWorkspaceId(params);
    return await listIssuesForCompany(ctx, workspaceId, getListLimit(params));
  });

  ctx.data.register("goals", async (params) => {
    const workspaceId = getCurrentWorkspaceId(params);
    return await listGoalsForCompany(ctx, workspaceId, getListLimit(params));
  });

  ctx.data.register("agents", async (params) => {
    const workspaceId = getCurrentWorkspaceId(params);
    return await ctx.agents.list({ workspaceId, limit: getListLimit(params), offset: 0 });
  });
  ...
  ctx.data.register("workspaces", async (params) => {
    ...
    if (!projectId) return [];
    return await ctx.projects.listWorkspaces(projectId, workspaceId);
  });
  ...
  ctx.data.register("comment-context", async (params) => {
    ...
    const comments = await ctx.issues.listComments(issueId, workspaceId);
  ...
  ctx.data.register("entity-context", async (params) => {
    ...
    if (!workspaceId || !entityId || !entityType) return null;

    if (entityType === "project") {
      return await ctx.projects.get(entityId, workspaceId);
    }
    if (entityType === "issue") {
      return await ctx.issues.get(entityId, workspaceId);
    }
    if (entityType === "goal") {
      return await ctx.goals.get(entityId, workspaceId);
    }
    if (entityType === "agent") {
      return await ctx.agents.get(entityId, workspaceId);
    }
    return { entityId, entityType, workspaceId };
  });
}
...
async function registerActionHandlers(ctx: PluginContext): Promise<void> {
  ctx.actions.register("emit-demo-event", async (params) => {
    ...
    await ctx.events.emit("demo-event", workspaceId, {
      message,
      source: "kitchen-sink",
    ...
    await ctx.telemetry.track("demo_event", {
      source: "manual",
      has_company: Boolean(workspaceId),
    });
    pushRecord({
      level: "info",
      source: "telemetry",
  ...
  ctx.actions.register("create-issue", async (params) => {
    ...
    const issue = await ctx.issues.create({ workspaceId, projectId, title, description });
    pushRecord({
      level: "info",
  ...
  ctx.actions.register("advance-issue-status", async (params) => {
    ...
    const issue = await ctx.issues.update(issueId, { status: status as Issue["status"] }, workspaceId);
    pushRecord({
      level: "info",
  ...
  ctx.actions.register("create-goal", async (params) => {
    ...
    const description = typeof params.description === "string" ? params.description : undefined;
    const goal = await ctx.goals.create({ workspaceId, title, description, level: "team", status: "planned" });
    pushRecord({
      level: "info",
      source: "goals.create",
  ...
  ctx.actions.register("advance-goal-status", async (params) => {
    ...
    const goal = await ctx.goals.update(goalId, { status: status as Goal["status"] }, workspaceId);
    pushRecord({
      level: "info",
      source: "goals.update",
  ...
  ctx.actions.register("run-process", async (params) => {
    ...
    const workspaceId = getCurrentWorkspaceId(params);
    const projectId = typeof params.projectId === "string" ? params.projectId : "";
    const projectWorkspaceId = typeof params.workspaceId === "string" && params.workspaceId.length > 0 ? params.workspaceId : undefined;
    const commandKey = typeof params.commandKey === "string" ? params.commandKey : "pwd";
    if (!projectId) throw new Error("projectId is required");
    return await runCuratedCommand(ctx, config, workspaceId, projectId, projectWorkspaceId, commandKey);
  });
  ...
  ctx.actions.register("start-progress-stream", async (params) => {
    ...
    void (async () => {
      ctx.streams.open(STREAM_CHANNELS.progress, workspaceId);
      try {
  ...
  ctx.actions.register("invoke-agent", async (params) => {
    ...
    const result = await ctx.agents.invoke(agentId, workspaceId, { prompt, reason: "Kitchen Sink plugin demo" });
    pushRecord({
      level: "info",
  ...
  ctx.actions.register("pause-agent", async (params) => {
    ...
    const agentId = typeof params.agentId === "string" ? params.agentId : "";
    if (!agentId) throw new Error("agentId is required");
    return await ctx.agents.pause(agentId, workspaceId);
  });
  ...
  ctx.actions.register("resume-agent", async (params) => {
    ...
    const agentId = typeof params.agentId === "string" ? params.agentId : "";
    if (!agentId) throw new Error("agentId is required");
    return await ctx.agents.resume(agentId, workspaceId);
  });
  ...
  ctx.actions.register("ask-agent", async (params) => {
    ...
    ctx.streams.open(STREAM_CHANNELS.agentChat, workspaceId);
    const session = await ctx.agents.sessions.create(agentId, workspaceId, {
      reason: "Kitchen Sink plugin chat demo",
    });

    await ctx.agents.sessions.sendMessage(session.sessionId, workspaceId, {
      prompt,
...
async function registerToolHandlers(ctx: PluginContext): Promise<void> {
  ...
  ctx.tools.register(
    ...
    async (_params, runCtx): Promise<ToolResult> => {
      const projects = await ctx.projects.list({ workspaceId: runCtx.workspaceId, limit: 50, offset: 0 });
      const issues = await ctx.issues.list({ workspaceId: runCtx.workspaceId, limit: 50, offset: 0 });
      const goals = await ctx.goals.list({ workspaceId: runCtx.workspaceId, limit: 50, offset: 0 });
      const agents = await ctx.agents.list({ workspaceId: runCtx.workspaceId, limit: 50, offset: 0 });
      return {
